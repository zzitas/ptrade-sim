            ...
            if last_minute is not None:
                ...
                try:
                    m = _pt_native("get_history")(1, frequency="1m",
                                                  ...
                                                  security_list=code,
                                                  include=True)
                    if m is not None and len(m)                             and float(m["close"].iloc[-1]) > 0:
            ...
            if isinstance(d2, dict) and d2:
                ...
                for f in flds:
                    if collected[f]:
                        got_any = True
                        per_field[f].append(_pd.DataFrame(collected[f]))
            elif isinstance(d2, _pd.DataFrame) and len(d2.columns)                     and len(flds) == 1:
...
def initialize(context):
    ...
    # ---------- 交易调度 ----------
    run_daily(context, pre_market_correlation_analysis, time='09:00')
    ...
    run_daily(context, reset_range_bound_daily, time='15:10')

    # ---------- 动态池调度 ----------
    run_daily(context, manage_dynamic_pool_lifecycle, time='09:05')
...
def market_broadcast(context, days=5):
    ...
    log.info("=" * 70)
    log.info(f"📢 全市场ETF行情播报（{days}日周期）- {context.blotter.current_dt.strftime('%Y-%m-%d %H:%M')}")
    log.info("=" * 70)
    ...
    if context.blotter.current_dt.hour >= 14:
...
def check_positions(context):
    log.info(f"\n======================🐂🧨🧨🧨🧨🧨{context.blotter.current_dt.strftime('%Y-%m-%d')}📌策略运行开始📌一路长红🧨🧨🧨🧨🧨🐂======================")
...
def is_etf_suspended(etf, context):
    ...
    try:
        ...
        minute_data = get_price(
            etf,
            start_date=today,
            end_date=context.blotter.current_dt,
            frequency='1m',
            fields=['volume'],
            skip_paused=False,
            panel=False
        ...
        if minute_data is None or minute_data.empty:
            if context.blotter.current_dt.hour >= 9:
        ...
        if check_profit_protection(sec, context):
            if smart_order_target_value(sec, 0, context):
                ...
                if getattr(g, 'enable_stop_loss_trigger', False):
                    ...
                    g.stop_loss_triggered_date = context.blotter.current_dt.date()
...
def get_risk_benchmark_state(context):
    ...
    try:
        today = context.blotter.current_dt.date()
        minute_df = get_price(
            g.risk_benchmark, start_date=today, end_date=context.blotter.current_dt,
...
def check_and_exit_range_bound_mode(context):
    ...
    try:
        ...
        if g.range_bound_start_date is not None:
            trade_days = get_trade_days(start_date=g.range_bound_start_date, end_date=context.blotter.current_dt.date())
        ...
        if should_recover:
            ...
            if g.last_switch_date is not None:
                trade_days = get_trade_days(start_date=g.last_switch_date, end_date=context.blotter.current_dt.date())
        ...
        if order_result:
            ...
            if diff > 0:
                g.trade_log['records'].append({
                    'time': context.blotter.current_dt.strftime('%Y-%m-%d %H:%M:%S'),
                    'security': security,
                    'name': name,
                    'direction': '买入',
                    'amount': abs(diff),
...
def calculate_momentum_metrics(context, etf):
    try:
        ...
        # 溢价率
        if g.enable_premium_filter:
            prev_date = get_trade_days(end_date=context.blotter.current_dt.date(), count=2)[0]
...
def get_volume_ratio(context, security, lookback=None, threshold=None):
    ...
    try:
        ...
        today = context.blotter.current_dt.date()
        df_vol = get_price(security, start_date=today, end_date=context.blotter.current_dt,
...
def daily_summary(context):
    log.info("=" * 70)
    log.info(f"📊 盘后总结 - {context.blotter.current_dt.strftime('%Y-%m-%d')}")
    log.info("=" * 70)
        ...
        for sec, pos in portfolio.positions.items():
            ...
            if hasattr(pos, 'init_time') and pos.init_time:
                try:
                    ...
                    end_date = context.blotter.current_dt.date()
...
def pre_market_correlation_analysis(context):
    ...
    log.info("=" * 70)
    log.info(f"📊 开盘前ETF相关性分析 - {context.blotter.current_dt.strftime('%Y-%m-%d %H:%M')}")
    log.info("=" * 70)
...
def dragon_stagnant_monitor(context):
    ...
    log.info("=" * 70)
    log.info(f"🔍 盘后龙头监测 - {context.blotter.current_dt.strftime('%Y-%m-%d %H:%M')}")
    log.info("=" * 70)
...
def reverse_repurchase(context):
    ...
    # 逆回购下单：卖出（-）对应张数
    try:
        order_id = order('131810.SZ', -1 * zhang_num)
        if order_id:
            log.info(f"【已完成逆回购】 品种：131810.SZ（R-001），下单张数：{zhang_num}，对应金额：{zhang_num * 100}元")
        else:
