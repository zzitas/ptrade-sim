# =====================================================================
# Ptrade 适配层（由 jq-to-pt 自动生成）
# 替代 JQ 专有 API: get_current_data / attribute_history / get_price(panel)
#                  get_all_securities / position 属性名 等
# =====================================================================
import math
import numpy as np
import pandas as pd
from datetime import datetime, date, timedelta
import datetime as _dt  # 隔离别名：适配层内部用 _dt.datetime/_dt.date，避免被用户 import datetime 遮蔽

# 当日价格缓存，避免重复拉取行情
_PRICE_CACHE = {}


def _fmt_dt(value, frequency='1d'):
    """将 date/datetime 转为 Ptrade 接受的字符串格式。

    陷阱：Ptrade 的 get_price 日期参数要求 YYYYmmdd（无横杠），ISO 的
    'YYYY-MM-DD' 会静默取数失败（见 user memory：get_price/get_index_stocks
    日期格式陷阱）。get_trade_days 两种格式都接受，故统一用 YYYYmmdd 即可。
    分钟线 end_date 带时间，格式为 'YYYYmmdd HH:MM'。
    """
    if value is None:
        return None
    if isinstance(value, (_dt.datetime, _dt.date)):
        if isinstance(value, _dt.datetime) and (value.hour, value.minute, value.second) != (0, 0, 0):
            if frequency.endswith('m'):
                return value.strftime('%Y%m%d %H:%M')
            return value.strftime('%Y%m%d')
        return value.strftime('%Y%m%d')
    if isinstance(value, str):
        # 已是字符串：若传入 ISO 'YYYY-MM-DD' 则去横杠转为 YYYYmmdd（防御）
        return value.replace('-', '')
    return value


def record(**kwargs):
    """JQ 的绘图 record 在 Ptrade 无等效，置为 no-op。"""
    return None


def jq_get_trade_days(start_date=None, end_date=None, count=None):
    """适配 get_trade_days 的日期参数类型，并归一化返回值为 datetime.date 列表（与 JQ 一致）。

    Ptrade 的 get_trade_days 在部分环境返回 numpy.str_/字符串，直接用于日期运算会触发
    TypeError: unsupported operand type(s) for -: 'numpy.str_' and 'datetime.timedelta'。
    这里统一转成 datetime.date，保证调用方可以安全做 date - timedelta 等运算。
    """
    sd = _fmt_dt(start_date) if start_date is not None else None
    ed = _fmt_dt(end_date) if end_date is not None else None
    days = get_trade_days(start_date=sd, end_date=ed, count=count)
    if days is None or (hasattr(days, '__len__') and len(days) == 0):
        return []
    result = []
    for d in days:
        if isinstance(d, _dt.datetime):
            result.append(d.date())
        elif isinstance(d, _dt.date):
            result.append(d)
        else:
            # numpy.str_ / str / numpy.datetime64 等：兼容 'YYYY-MM-DD' 与 'YYYYmmdd'
            s = str(d)
            result.append(_dt.datetime.strptime(s[:10].replace('-', ''), '%Y%m%d').date())
    return result


def jq_attribute_history(security, count, unit, fields, skip_paused=False):
    """JQ attribute_history -> Ptrade get_history（单标的多字段返回 DataFrame）。

    ⚠️ 前复权陷阱修复：include=False 拿到的历史K线复权基准可能与当日实时价
    （last_price，未复权）不一致——做过份额折算/拆分的ETF，历史价被÷N 而
    实时价不缩放，拼接后涨幅爆炸（如7日513%）。
    方案：统一用 include=True 取数（fq='pre' 复权链与当日对齐），再砍掉
    最后一根当日K线，恢复 JQ attribute_history「不含当日」语义；
    需要"当日价"的调用方用 jq_last_bar() 取同一复权链下、上一分钟已收盘的K线（避免未来函数）。
    """
    df = get_history(count + 1, frequency=unit, field=fields,
                     security_list=security, fq='pre', include=True)
    try:
        if df is not None and hasattr(df, '__len__') and len(df) > 0:
            df = df.iloc[:-1]  # 去掉当日数据，保持「截至昨日」语义
    except Exception:
        pass
    return df


def jq_last_bar(security, unit='1m', fields=('close',)):
    """取上一根已完成的分钟K线（前复权），用于分析口径拼接当日最新价。

    与 jq_attribute_history 保持同一复权链（fq='pre'）；关键是用 include=True
    取最近 2 根 1m（含当前正在形成的bar），再 iloc[:-1] 去掉当前bar，
    保留上一分钟已收盘数据——避免把当日日K线收盘（含未来信息）当当日价拼入序列。
    仅用于分析口径；下单路径仍必须用 last_price 等交易所原始价。
    """
    try:
        df = get_history(2, frequency=unit, field=list(fields),
                         security_list=security, fq='pre', include=True)
        if df is None or (hasattr(df, 'empty') and df.empty):
            return None
        df = df.iloc[:-1]   # 去掉当前正在形成的分钟bar，保留上一分钟
        if len(df) == 0:
            return None
        return df
    except Exception:
        return None


def _single_field_df(security, frequency, field, dt):
    """单字段行情获取，返回末尾值（避免 Panel，逐字段请求）。

    兼容 PTrade get_price 的宽表 / 长表两种返回结构；回退时尊重调用方周期——
    盘中最新价（'close'）优先走分钟K线，分钟不可用时退日线收盘（含当日，非未来函数），
    避免返回 0.0 导致动量计算 np.log(0)=-inf 变 nan。
    """
    try:
        df = get_price([security], end_date=dt, count=1, frequency=frequency,
                       fields=field, fq='pre')
    except Exception:
        df = None
    if df is None or (hasattr(df, 'empty') and df.empty):
        # 回退：尊重调用方周期——盘中最新价走分钟K线（'1m'），开盘/涨跌停价走日线（'1d'）。
        # 价格类字段在分钟仍取不到时，再退日线收盘（含当日，非未来函数）兜底，避免返回 0.0。
        freqs = [frequency]
        if field in ('close', 'pre_close', 'last_price'):
            freqs.append('1d')
        for fb_freq in freqs:
            try:
                d2 = get_history(1, fb_freq, field, security_list=[security], include=True)
                if d2 is not None and not d2.empty:
                    if security in d2.columns:
                        return float(d2[security].iloc[-1])
                    if field in d2.columns:
                        return float(d2[field].iloc[-1])
            except Exception:
                continue
        return 0.0
    try:
        # 兼容宽表（columns=代码）与长表（含 'code' 列）
        if security in df.columns:
            return float(df[security].iloc[-1])
        if 'code' in df.columns:
            sub = df[df['code'] == security]
            if not sub.empty and field in sub.columns:
                return float(sub[field].iloc[-1])
        if field in df.columns:
            return float(df[field].iloc[-1])
    except Exception:
        pass
    return 0.0


class _SecurityData:
    """模拟 JQ SecurityData：paused/last_price/day_open/high_limit/low_limit。"""

    def __init__(self, security):
        ctx = getattr(g, 'context', None)
        if ctx is None:
            self.paused = False
            self.last_price = 0.0
            self.day_open = 0.0
            self.high_limit = float('inf')
            self.low_limit = -float('inf')
            return
        dt_str = ctx.current_dt.strftime('%Y-%m-%d')
        key = (security, dt_str)
        cached = _PRICE_CACHE.get(key)
        if cached is not None:
            self.paused = cached['paused']
            self.last_price = cached['last_price']
            self.day_open = cached['day_open']
            self.high_limit = cached['high_limit']
            self.low_limit = cached['low_limit']
            return
        last_price = _single_field_df(security, '1m', 'close', ctx.current_dt)
        day_open = _single_field_df(security, '1d', 'open', ctx.current_dt)
        hl = _single_field_df(security, '1d', 'high_limit', ctx.current_dt)
        ll = _single_field_df(security, '1d', 'low_limit', ctx.current_dt)
        high_limit = hl if (hl and hl > 0 and not math.isnan(hl)) else float('inf')
        low_limit = ll if (ll and ll < 0 and not math.isnan(ll)) else -float('inf')
        paused = False
        try:
            # 陷阱15：get_stock_status 在回测中会静默挂起（阻塞非异常，try/except 拦不住）。
            # 回测路径直接置未停牌，避免构造 _SecurityData 时卡死引擎。
            if is_trade():
                st = get_stock_status([security], query_type='HALT')
                if isinstance(st, dict):
                    paused = bool(st.get(security, False))
        except Exception:
            paused = False
        self.paused = paused
        self.last_price = last_price
        self.day_open = day_open
        self.high_limit = high_limit
        self.low_limit = low_limit
        _PRICE_CACHE[key] = {'paused': paused, 'last_price': last_price,
                             'day_open': day_open, 'high_limit': high_limit,
                             'low_limit': low_limit}


class _CurrentDataProxy:
    def __init__(self, context):
        self._ctx = context

    def __getitem__(self, security):
        return _SecurityData(security)


def get_current_data():
    """替代 JQ get_current_data()，返回 {security: _SecurityData} 代理。"""
    return _CurrentDataProxy(getattr(g, 'context', None))


def jq_get_price(security, end_date=None, count=None, start_date=None,
                 frequency='1d', fields=None, fq='pre', **kwargs):
    """替代 JQ get_price(..., panel=False)：返回长表
    列=['time','code', *fields]，与 JQ panel=False 行为一致。"""
    end_date = _fmt_dt(end_date, frequency)
    start_date = _fmt_dt(start_date, frequency)
    # Ptrade 不允许 start_date 等于/晚于当前交易日，否则报
    # "start_date大于等于回测或交易日期"；将「同日盘中/当日」请求改为 count 模式。
    if start_date is not None and end_date is not None:
        try:
            sd_d = _dt.datetime.strptime(start_date.split(' ')[0], '%Y%m%d').date()
            ed_full = end_date if ' ' in end_date else end_date + ' 15:00'
            ed_dt = _dt.datetime.strptime(ed_full, '%Y%m%d %H:%M')
            if sd_d >= ed_dt.date():
                if frequency.endswith('m'):
                    open_dt = ed_dt.replace(hour=9, minute=31, second=0, microsecond=0)
                    count = max(1, int((ed_dt - open_dt).total_seconds() // 60) + 1)
                else:
                    count = 1
                start_date = None
        except Exception:
            pass
    if fields is None:
        fields = ['close']
    if isinstance(fields, str):
        fields = [fields]
    if isinstance(security, str):
        security = [security]
    sec_list = list(security)
    frames = []
    for fld in fields:
        try:
            df = get_price(sec_list, end_date=end_date, count=count,
                           start_date=start_date, frequency=frequency,
                           fields=fld, fq=fq)
        except Exception:
            df = None
        if df is None or (hasattr(df, 'empty') and df.empty):
            sub = pd.DataFrame(columns=['time', 'code', fld])
        else:
            d2 = df.reset_index()
            cols = list(d2.columns)
            if 'code' in cols:
                # 已是长表：get_price 直接返回 code / 日期 / 字段 三列，
                # 切勿再 melt，否则会把 'code' 字面量熔进 code 列，
                # 导致下游 pivot 因 (time, code) 重复键崩溃。
                date_candidates = [c for c in cols if c not in ('code', fld, 'time', 'index')]
                if 'time' in cols:
                    pass
                elif date_candidates:
                    d2 = d2.rename(columns={date_candidates[0]: 'time'})
                elif 'index' in cols:
                    d2 = d2.rename(columns={'index': 'time'})
                keep = [c for c in ('time', 'code', fld) if c in d2.columns]
                sub = d2[keep].copy()
            else:
                # 宽表：index=日期, columns=各标的代码 -> melt 转长表
                if 'time' not in cols:
                    rename_from = 'index' if 'index' in cols else cols[0]
                    d2 = d2.rename(columns={rename_from: 'time'})
                sub = d2.melt(id_vars='time', var_name='code', value_name=fld)
                sub = sub[['time', 'code', fld]]
            # PTrade get_price 返回的行情字段可能为 object/字符串，强制转数值，
            # 否则下游 groupby.sum() 会对字符串拼接，导致 / 运算触发 str/int 类型错误。
            sub[fld] = pd.to_numeric(sub[fld], errors='coerce')
        frames.append(sub)
    if not frames:
        return pd.DataFrame(columns=['time', 'code'] + list(fields))
    long = frames[0]
    for nxt in frames[1:]:
        long = pd.merge(long, nxt, on=['time', 'code'], how='outer')
    return long


# =====================================================================
# 克隆自聚宽文章：https://www.joinquant.com/post/76560
# 标题：（目前最强七星：今年487%）PT自动打新和逆回购
# 作者：开心etf

# 克隆自聚宽文章：https://www.joinquant.com/post/76366
# 标题：动量反写邪恶七星日志加强版（可用）
# 作者：偷鸡万岁


# ==================== 📦 导入模块 ====================
import numpy as np
import math
import datetime
import time
import pandas as pd
from functools import wraps

# ==================== 🌟 全局 ETF 池配置 ====================
MASTER_ETF_POOL = [
        # 行业板块 (后视镜)
        "515880.SS",  # 通信ETF
        "159516.SZ",  # 半导体设备
        "159530.SZ",  # 机器人
        "159206.SZ",  # 卫星ETF永赢
        "560390.SS",  # 电网设备ETF
        "561910.SS",  # 电池ETF
        "159381.SZ",  # 创业板人工智能ETF
        "159153.SZ",  # 消费电子ETF鹏华
        "159783.SZ",  # 科创创业50ETF
        "588710.SS",  # 科半导体
        "588220.SS",  # 科创100F
        # 大宗商品ETF
        "518880.SS",  # 黄金ETF
        "159980.SZ",  # 有色ETF
        "159985.SZ",  # 豆粕ETF
        "501018.SS",  # 南方原油
        '161226.SZ',  # 白银LOF
        "159981.SZ",  # 能源化工ETF
        # 国际ETF
        "513100.SS",  # 纳指ETF
        "159509.SZ",  # 纳指科技ETF
        "513290.SS",  # 纳指生物ETF
        "513500.SS",  # 标普500ETF
        "159529.SZ",  # 标普消费
        "513400.SS",  # 道琼斯ETF
        "513520.SS",  # 日经225ETF
        "513030.SS",  # 德国30ETF
        "513080.SS",  # 法国ETF
        "513310.SS",  # 中韩半导体ETF
        "513730.SS",  # 东南亚ETF
        # 香港ETF
        "159792.SZ",  # 港股互联ETF
        "513130.SS",  # 恒生科技
        "513050.SS",  # 中概互联网ETF
        "159920.SZ",  # 恒生ETF
        "513690.SS",  # 港股红利
        # 指数ETF
        "510300.SS",  # 沪深300ETF
        "510500.SS",  # 中证500ETF
        "510050.SS",  # 上证50ETF
        "510210.SS",  # 上证ETF
        "159915.SZ",  # 创业板ETF
        "588080.SS",  # 科创50
        "512100.SS",  # 中证1000ETF
        "563360.SS",  # A500-ETF
        "563300.SS",  # 中证2000ETF
        # 风格ETF
        "512890.SS",  # 红利低波ETF
        "159967.SZ",  # 创业板成长ETF
        "512040.SS",  # 价值ETF
        "159201.SZ",  # 自由现金流ETF
        # 债券ETF
        "511380.SS",  # 可转债ETF
        "511010.SS",  # 国债ETF
        "511220.SS",  # 城投债ETF
    ]

# ==================== 🌟 行业ETF池（约421只，仅供播报） ====================
INDUSTRY_ETF_POOL = [
    # ==================== 芯片半导体 ====================
    "513310.SS",   # 中韩半导体ETF华泰柏瑞
    "588260.SS",   # 科创信息ETF华安
    "159801.SZ",   # 芯片ETF广发
    "588200.SS",   # 科创芯片ETF嘉实
    "588170.SS",   # 科创半导体ETF华夏
    "159516.SZ",   # 半导体设备ETF国泰
    "159995.SZ",   # 芯片ETF华夏
    "512480.SS",   # 半导体ETF国联安
    "159558.SZ",   # 半导体设备ETF易方达
    "512760.SS",   # 芯片ETF国泰
    "562590.SS",   # 半导体设备ETF华夏
    "588290.SS",   # 科创芯片ETF华安
    "159813.SZ",   # 半导体ETF鹏华
    "588790.SS",   # 科创AIETF博时
    "588710.SS",   # 科创半导体设备ETF华泰柏瑞
    "560780.SS",   # 半导体设备ETF广发
    "588750.SS",   # 科创芯片ETF汇添富
    "561980.SS",   # 半导体设备ETF招商
    "589020.SS",   # 科创半导体设备ETF鹏华
    "588780.SS",   # 科创芯片设计ETF国联安
    "588890.SS",   # 科创芯片ETF南方
    "159325.SZ",   # 半导体ETF南方
    "159327.SZ",   # 半导体设备ETF万家
    "588810.SS",   # 科创芯片ETF富国
    "588990.SS",   # 科创芯片ETF博时
    "159546.SZ",   # 集成电路ETF国泰
    "159665.SZ",   # 半导体龙头ETF工银
    "159582.SZ",   # 半导体ETF博时
    "562820.SS",   # 集成电路ETF嘉实
    "159560.SZ",   # 芯片ETF景顺
    "159310.SZ",   # 芯片ETF天弘
    "516350.SS",   # 芯片ETF易方达
    "159599.SZ",   # 芯片ETF东财
    "516640.SS",   # 芯片ETF富国
    "516920.SS",   # 芯片ETF汇添富
    "589100.SS",   # 科创芯片ETF国泰
    "588920.SS",   # 科创芯片ETF鹏华
    "588100.SS",   # 科创信息ETF嘉实
    "588770.SS",   # 科创信息ETF摩根
    "512330.SS",   # 信息科技ETF南方
    "159939.SZ",   # 信息技术ETF广发
    "159997.SZ",   # 电子ETF天弘
    "515260.SS",   # 电子ETF华宝
    "515320.SS",   # 电子50ETF华安
    
    # ==================== 通信 ====================
    "560690.SS",   # 电信ETF鹏华
    "560300.SS",   # 电信ETF汇添富
    "515880.SS",   # 通信ETF国泰
    "515050.SS",   # 通信ETF华夏
    "159583.SZ",   # 通信ETF富国
    "159994.SZ",   # 通信ETF银华
    "159695.SZ",   # 通信ETF嘉实
    "159507.SZ",   # 通信ETF广发
    "159511.SZ",   # 通信ETF南方
    "159811.SZ",   # 5GETF博时
    "563010.SS",   # 电信ETF易方达
    
    # ==================== 证券/券商 ====================
    "513090.SS",   # 香港证券ETF易方达
    "516730.SS",   # 证券ETF浦银
    "159848.SZ",   # 证券ETF国联安
    "510200.SS",   # 上证券商ETF汇安
    "516980.SS",   # 证券先锋ETF华富
    "562870.SS",   # 证券ETF嘉实
    "516200.SS",   # 证券ETF华安
    "512880.SS",   # 证券ETF国泰
    "512000.SS",   # 券商ETF华宝
    "159842.SZ",   # 券商ETF银华
    "159841.SZ",   # 证券ETF天弘
    "512570.SS",   # 证券ETF易方达
    "560090.SS",   # 证券ETF汇添富
    "515010.SS",   # 证券ETF华夏
    "159993.SZ",   # 证券ETF鹏华
    "159692.SZ",   # 证券ETF东财
    "512900.SS",   # 证券ETF南方
    "515850.SS",   # 证券ETF富国
    "515560.SS",   # 证券ETF建信
    "512070.SS",   # 证券保险ETF易方达
    "515630.SS",   # 证券保险ETF鹏华
    
    # ==================== 银行 ====================
    "512800.SS",   # 银行ETF华宝
    "516310.SS",   # 银行ETF易方达
    "515020.SS",   # 银行ETF华夏
    "159887.SZ",   # 银行ETF富国
    "512820.SS",   # 银行ETF汇添富
    "512700.SS",   # 银行ETF南方
    "515290.SS",   # 银行ETF天弘
    "512730.SS",   # 银行ETF鹏华
    "516210.SS",   # 银行ETF华安
    "517900.SS",   # 银行AH优选ETF招商
    # ==================== 金融/金融科技 ====================
    "159933.SZ",   # 金融地产ETF国投瑞银
    "563670.SS",   # 金融科技ETF鹏华
    "515720.SS",   # 金融科技ETF富国
    "159103.SZ",   # 金融科技ETF汇添富
    "510650.SS",   # 金融地产ETF华夏
    "510230.SS",   # 金融ETF国泰
    "159931.SZ",   # 金融地产ETF汇添富
    "513750.SS",   # 港股通非银ETF广发
    "159940.SZ",   # 金融地产ETF广发
    "512640.SS",   # 金融地产ETF嘉实
    
    # ==================== 电力/公用事业 ====================
    "560620.SS",   # 公用事业ETF万家
    "560190.SS",   # 公用事业ETF鹏华
    "159669.SZ",   # 绿色电力ETF国泰
    "512580.SS",   # 环保ETF广发
    "159611.SZ",   # 电力ETF广发
    "561560.SS",   # 电力ETF华泰柏瑞
    "562550.SS",   # 绿电ETF华夏
    "562960.SS",   # 绿色电力ETF易方达
    "561170.SS",   # 绿色电力ETF富国
    "159625.SZ",   # 绿色电力ETF嘉实
    "562350.SS",   # 电力ETF银华
    "561700.SS",   # 电力ETF博时
    "560580.SS",   # 电力ETF南方
    "159301.SZ",   # 公用事业ETF华夏
    "159320.SZ",   # 电网设备ETF广发
    "561380.SS",   # 电网设备ETF国泰
    "159326.SZ",   # 电网设备ETF华夏
    
    # ==================== 红利 ====================
    "159336.SZ",   # 央企红利ETF融通
    "159332.SZ",   # 央企红利ETF富国
    "512390.SS",   # 中国低波ETF平安
    "562060.SS",   # 标普A股红利ETF华宝
    "561680.SS",   # A500红利低波ETF平安
    "560700.SS",   # 央企红利ETF广发
    "512530.SS",   # 沪深300红利ETF建信
    "159296.SZ",   # A500红利低波ETF华宝
    "159228.SZ",   # 红利低波ETF长城
    "560520.SS",   # 红利低波100ETF大成
    "560070.SS",   # 央企红利ETF汇添富
    "159515.SZ",   # 国企红利ETF鹏扬
    "159333.SZ",   # 港股央企红利ETF万家
    "159355.SZ",   # 800红利低波ETF华宝
    "560150.SS",   # 红利低波ETF泰康
    "563690.SS",   # 红利低波ETF永赢
    "563890.SS",   # 国企红利ETF创金合信
    "159589.SZ",   # 红利ETF广发
    "563700.SS",   # XD红利价值ETF易方达
    "512890.SS",   # 红利低波ETF华泰柏瑞
    "510880.SS",   # 红利ETF华泰柏瑞
    "515180.SS",   # 红利ETF易方达
    "515080.SS",   # 中证红利ETF招商
    "515450.SS",   # 红利低波50ETF南方
    "515100.SS",   # 红利低波100ETF景顺
    "159307.SZ",   # 红利低波100ETF博时
    "515300.SS",   # 300红利低波ETF嘉实
    "159549.SZ",   # 红利低波ETF天弘
    "159525.SZ",   # 红利低波ETF富国
    "510720.SS",   # 红利国企ETF国泰
    "530880.SS",   # 红利国企ETF银河
    "561580.SS",   # 央企红利ETF华泰柏瑞
    "159905.SZ",   # 红利ETF工银
    "515890.SS",   # 红利ETF博时
    "159581.SZ",   # 红利ETF万家
    "560570.SS",   # A500红利ETF国联安
    "561060.SS",   # 国企红利ETF华安
    "159209.SZ",   # 红利质量ETF招商
    "159758.SZ",   # 红利质量ETF华夏
    "159708.SZ",   # 红利ETF西部利得
    
    # ==================== 消费/食品饮料 ====================
    "159725.SZ",   # 线上消费ETF工银
    "159672.SZ",   # 消费ETF博时
    "560680.SS",   # 消费ETF广发
    "159728.SZ",   # 在线消费ETF南方
    "516900.SS",   # 食品饮料ETF华安
    "159689.SZ",   # 消费ETF南方
    "512600.SS",   # 消费ETF嘉实
    "159843.SZ",   # 食品饮料ETF招商
    "159793.SZ",   # 线上消费ETF平安
    "159520.SZ",   # 消费龙头ETF工银
    "517550.SS",   # 沪港深消费龙头ETF招商
    "159862.SZ",   # 食品饮料ETF银华
    "159730.SZ",   # 家电ETF博时
    "159936.SZ",   # 可选消费ETF广发
    "562580.SS",   # 可选消费ETF华夏
    "159670.SZ",   # 消费50ETF国联安
    "516600.SS",   # 消费服务ETF工银
    "159699.SZ",   # 恒生消费ETF广发
    "561120.SS",   # 家电ETF富国
    "517880.SS",   # 品牌消费ETF华泰柏瑞
    "159328.SZ",   # 家电ETF易方达
    "159529.SZ",   # 标普消费ETF景顺
    "515920.SS",   # 智能消费ETF博时
    "512690.SS",   # 酒ETF鹏华
    "159928.SZ",   # 消费ETF汇添富
    "515170.SS",   # 食品饮料ETF华夏
    "515710.SS",   # 食品饮料ETF华宝
    "159736.SZ",   # 食品饮料ETF天弘
    "515650.SS",   # 消费50ETF富国
    "510630.SS",   # 消费ETF华夏
    "510150.SS",   # 消费ETF招商
    "159798.SZ",   # 消费ETF易方达
    "561130.SS",   # 国货ETF富国
    "516130.SS",   # 消费龙头ETF华宝
    "159732.SZ",   # 消费电子ETF华夏
    "562950.SS",   # 消费电子ETF易方达
    "561100.SS",   # 消费电子ETF富国
    "561600.SS",   # 消费电子ETF平安
    "159779.SZ",   # 消费电子ETF招商
    "561310.SS",   # 消电ETF国泰
    "159996.SZ",   # 家电ETF国泰
    "560880.SS",   # 家电ETF广发
    
    # ==================== 医药/医疗 ====================
    "159643.SZ",   # 疫苗ETF国泰
    "159567.SZ",   # 港股创新药ETF银华
    "159657.SZ",   # 疫苗ETF鹏华
    "159508.SZ",   # 生物医药ETF华安
    "562860.SS",   # 疫苗ETF嘉实
    "159506.SZ",   # 港股通创新药医疗ETF富国
    "159645.SZ",   # 疫苗ETF富国
    "513120.SS",   # 港股创新药ETF广发
    "561920.SS",   # 疫苗ETF招商
    "520700.SS",   # 港股通创新药ETF万家
    "159366.SZ",   # 港股医疗ETF永赢
    "560900.SS",   # 创新药ETF摩根
    "517120.SS",   # 创新药ETF华泰柏瑞
    "159849.SZ",   # 生物科技ETF招商
    "516930.SS",   # 生物科技ETF民生加银
    "560260.SS",   # 医疗ETF广发
    "517990.SS",   # 沪港深医药ETF招商
    "588250.SS",   # 科创医药ETF鹏华
    "515950.SS",   # 医药50ETF富国
    "159891.SZ",   # 医疗ETF建信
    "159760.SZ",   # 医疗健康ETF泰康
    "159797.SZ",   # 医疗器械ETF汇添富
    "515960.SS",   # 医药ETF嘉实
    "561510.SS",   # 中药ETF华泰柏瑞
    "159873.SZ",   # 医疗设备ETF天弘
    "562390.SS",   # 中药ETF银华
    "159877.SZ",   # 医疗ETF南方
    "516790.SS",   # 医疗ETF华泰柏瑞
    "516610.SS",   # 医疗设备ETF大成
    "159838.SZ",   # 医药50ETF博时
    "513290.SS",   # 纳指生物科技ETF汇添富
    "512170.SS",   # 医疗ETF华宝
    "512010.SS",   # 医药ETF易方达
    "159992.SZ",   # 创新药ETF银华
    "515120.SS",   # 创新药ETF广发
    "159859.SZ",   # 生物医药ETF天弘
    "512290.SS",   # 生物医药ETF国泰
    "159839.SZ",   # 生物医药ETF汇添富
    "159929.SZ",   # 医药ETF汇添富
    "159938.SZ",   # 医药ETF广发
    "512120.SS",   # 医药ETF华安
    "510660.SS",   # 医药ETF华夏
    "159837.SZ",   # 生物科技ETF易方达
    "516500.SS",   # 生物科技ETF华夏
    "159883.SZ",   # 医疗器械ETF永赢
    "562600.SS",   # 医疗器械ETF华夏
    "159898.SZ",   # 医疗器械ETF招商
    "159828.SZ",   # 医疗ETF国泰
    "159847.SZ",   # 医疗ETF易方达
    "516820.SS",   # 医疗创新ETF平安
    "159622.SZ",   # 创新药ETF东财
    "159748.SZ",   # 创新药ETF富国
    "159858.SZ",   # 创新药ETF南方
    "159835.SZ",   # 创新药ETF建信
    "517380.SS",   # 创新药ETF天弘
    "516080.SS",   # 创新药ETF易方达
    "517110.SS",   # 创新药ETF国泰
    "516060.SS",   # 创新药ETF工银
    "589720.SS",   # 科创创新药ETF国泰
    "589120.SS",   # 科创创新药ETF汇添富
    "560080.SS",   # 中药ETF汇添富
    "159647.SZ",   # 中药ETF鹏华
    "588700.SS",   # 科创医药ETF嘉实
    "588130.SS",   # 科创医药ETF华夏
    "588860.SS",   # 科创医药ETF工银
    "159377.SZ",   # 创业板医药ETF国泰
    "562050.SS",   # 药ETF华宝
    
    # ==================== 新能源/光伏/电池 ====================
    "159872.SZ",   # 智能网联汽车ETF鹏华
    "159618.SZ",   # 光伏ETF华安
    "159795.SZ",   # 智能汽车ETF汇添富
    "159889.SZ",   # 智能汽车ETF国泰
    "159863.SZ",   # 光伏ETF鹏华
    "159609.SZ",   # 光伏ETF浦银
    "516180.SS",   # 光伏ETF平安
    "159637.SZ",   # 新能源车ETF东财
    "516390.SS",   # 新能源车ETF汇添富
    "159767.SZ",   # 电池龙头ETF兴银
    "516590.SS",   # 智能汽车ETF易方达
    "159824.SZ",   # 新能源车ETF博时
    "516660.SS",   # 新能源车ETF华安
    "516380.SS",   # 智能电动车ETF华宝
    "516580.SS",   # 新能源ETF博时
    "159752.SZ",   # 新能源ETF申万菱信
    "159261.SZ",   # 创业板新能源ETF鹏华
    "515790.SS",   # 光伏ETF华泰柏瑞
    "159857.SZ",   # 光伏ETF天弘
    "159864.SZ",   # 光伏ETF国泰
    "516880.SS",   # 光伏ETF银华
    "516290.SS",   # 光伏ETF汇添富
    "562970.SS",   # 光伏ETF易方达
    "560980.SS",   # 光伏龙头ETF广发
    "515030.SS",   # 新能源车ETF华夏
    "515700.SS",   # 新能源车ETF平安
    "159806.SZ",   # 新能源车ETF国泰
    "159755.SZ",   # 电池ETF广发
    "159796.SZ",   # 电池ETF汇添富
    "562880.SS",   # 电池ETF嘉实
    "561910.SS",   # 电池ETF招商
    "561160.SS",   # 电池ETF富国
    "159775.SZ",   # 电池ETF建信
    "159757.SZ",   # 电池ETF景顺
    "159566.SZ",   # 储能电池ETF易方达
    "159305.SZ",   # 储能电池ETF广发
    "159840.SZ",   # 锂电池ETF工银
    "159790.SZ",   # 碳中和ETF华夏
    "516160.SS",   # 新能源ETF南方
    "516270.SS",   # 新能源ETF华安
    "159875.SZ",   # 新能源ETF嘉实
    "516850.SS",   # 新能源ETF华夏
    "516090.SS",   # 新能源ETF易方达
    "159387.SZ",   # 创业板新能源ETF国泰
    "159368.SZ",   # 创业板新能源ETF华夏
    "588830.SS",   # 科创新能源ETF鹏华
    "588960.SS",   # 科创新能源ETF富国
    "589960.SS",   # 科创新能源ETF易方达
    # ==================== 汽车/汽车零部件/智能驾驶 ====================
    "159306.SZ",   # 汽车零部件ETF平安
    "159720.SZ",   # 智能车ETF泰康
    "159512.SZ",   # 汽车ETF广发
    "516110.SS",   # 汽车ETF国泰
    "159565.SZ",   # 汽车零部件ETF易方达
    "562700.SS",   # 汽车零部件ETF华夏
    "516520.SS",   # 智能驾驶ETF华泰柏瑞
    "159888.SZ",   # 智能汽车ETF华夏
    "515250.SS",   # 智能汽车ETF富国
    
    # ==================== 有色/稀土/稀有金属 ====================
    "561330.SS",   # 矿业ETF国泰
    "512400.SS",   # 有色金属ETF南方
    "159652.SZ",   # 有色ETF汇添富
    "159876.SZ",   # 有色ETF华宝
    "159880.SZ",   # 有色ETF鹏华
    "159871.SZ",   # 有色ETF银华
    "159881.SZ",   # 有色金属ETF国泰
    "516650.SS",   # 有色金属ETF华夏
    "562800.SS",   # 稀有金属ETF嘉实
    "159608.SZ",   # 稀有金属ETF广发
    "159671.SZ",   # 稀有金属ETF工银
    "561800.SS",   # 稀有金属ETF华富
    "516150.SS",   # 稀土ETF嘉实
    "159715.SZ",   # 稀土ETF易方达
    "159713.SZ",   # 稀土ETF富国
    "516780.SS",   # 稀土ETF华泰柏瑞
    "159690.SZ",   # 有色矿业ETF招商
    "560860.SS",   # 工业有色ETF万家
    # ==================== 钢铁/材料 ====================
    "515210.SS",   # 钢铁ETF国泰
    "159944.SZ",   # 材料ETF广发
    "516710.SS",   # 新材料ETF华夏
    "159763.SZ",   # 新材料ETF建信
    "516890.SS",   # 新材料ETF平安
    "516360.SS",   # 新材料ETF华宝
    "159703.SZ",   # 新材料ETF天弘
    "159761.SZ",   # 新材料ETF国泰
    
    # ==================== 化工 ====================
    "159870.SZ",   # 化工ETF鹏华
    "516020.SS",   # 化工ETF华宝
    "516120.SS",   # 化工ETF富国
    "516220.SS",   # 化工ETF国泰
    "516570.SS",   # 化工行业ETF易方达
    
    # ==================== 人工智能/云计算/软件 ====================
    "159613.SZ",   # 信息安全ETF嘉实
    "516700.SS",   # 大数据ETF华宝
    "560360.SS",   # 软件ETF万家
    "562920.SS",   # 信息安全ETF易方达
    "517390.SS",   # 云计算ETF天弘
    "159311.SZ",   # 数字经济ETF易方达
    "159248.SZ",   # 人工智能ETF万家
    "589380.SS",   # 科创人工智能ETF富国
    "561220.SS",   # 数字经济ETF工银
    "159363.SZ",   # 创业板人工智能ETF华宝
    "159819.SZ",   # 人工智能ETF易方达
    "159381.SZ",   # 创业板人工智能ETF华夏
    "515980.SS",   # 人工智能ETF华富
    "515070.SS",   # 人工智能ETF华夏
    "159246.SZ",   # 创业板人工智能ETF富国
    "159382.SZ",   # 创业板人工智能ETF南方
    "588760.SS",   # 科创人工智能ETF广发
    "588730.SS",   # 科创人工智能ETF易方达
    "159242.SZ",   # 创业板人工智能ETF大成
    "159388.SZ",   # 创业板人工智能ETF国泰
    "159279.SZ",   # 创业板人工智能ETF华安
    "589010.SS",   # 科创人工智能ETF华夏
    "588930.SS",   # 科创人工智能ETF银华
    "589520.SS",   # 科创人工智能ETF华宝
    "589560.SS",   # 科创人工智能ETF汇添富
    "589110.SS",   # 科创人工智能ETF国泰
    "589090.SS",   # 科创AIETF鹏华
    "512930.SS",   # AI人工智能ETF平安
    "515000.SS",   # 科技ETF华宝
    "159807.SZ",   # 科技ETF易方达
    "515750.SS",   # 科技50ETF富国
    "516050.SS",   # 科技龙头ETF工银
    "517800.SS",   # 人工智能50ETF方正富邦
    "512220.SS",   # TMTETF景顺
    "515580.SS",   # 科技100ETF华泰柏瑞
    "159852.SZ",   # 软件ETF嘉实
    "515230.SS",   # 软件ETF国泰
    "159899.SZ",   # 软件ETF招商
    "562930.SS",   # 软件ETF易方达
    "561010.SS",   # 软件ETF华安
    "159590.SZ",   # 软件ETF汇添富
    "516510.SS",   # 云计算ETF易方达
    "516630.SS",   # 云计算ETF华夏
    "159739.SZ",   # 云计算ETF鹏华
    "159273.SZ",   # 云计算ETF汇添富
    "159738.SZ",   # 云计算ETF华泰柏瑞
    "159890.SZ",   # 云计算ETF招商
    "159527.SZ",   # 云计算ETF广发
    "560660.SS",   # 云计算50ETF新华
    "515400.SS",   # 大数据ETF富国
    "516000.SS",   # 大数据ETF华夏
    "159998.SZ",   # 计算机ETF天弘
    "512720.SS",   # 计算机ETF国泰
    "159851.SZ",   # 金融科技ETF华宝
    "516860.SS",   # 金融科技ETF博时
    "516100.SS",   # 金融科技ETF华夏
    "159299.SZ",   # 金融科技ETF易方达
    "562570.SS",   # 信创ETF华夏
    "159538.SZ",   # 信创ETF富国
    "159539.SZ",   # 信创ETF广发
    "159537.SZ",   # 信创ETF国泰
    "562030.SS",   # 信创ETF华宝
    "560850.SS",   # 信创ETF汇添富
    "159540.SZ",   # 信创ETF易方达
    "560800.SS",   # 数字经济ETF鹏扬
    "159385.SZ",   # 数字经济ETF富国
    "159658.SZ",   # 数字经济ETF华安
    "159389.SZ",   # 数字经济ETF嘉实
    "159256.SZ",   # 创业板软件ETF华夏
    "159107.SZ",   # 创业板软件ETF富国
    "563210.SS",   # 专精特新ETF国富
    # ==================== 科技/计算机/互联网/物联网 ====================
    "159586.SZ",   # 计算机ETF南方
    "517360.SS",   # 沪港深科技ETF华安
    "517350.SS",   # 沪港深科技ETF广发
    "515860.SS",   # 科技ETF嘉实
    "159723.SZ",   # 科技龙头ETF汇添富
    "159777.SZ",   # 创科技ETF国联安
    "159773.SZ",   # 创业板科技ETF华泰柏瑞
    "560990.SS",   # 科技先锋ETF中金
    "562560.SS",   # 信息技术ETF华夏
    "159729.SZ",   # 互联网ETF汇添富
    "159550.SZ",   # 互联网ETF东财
    "517050.SS",   # 互联网ETF华泰柏瑞
    "517200.SS",   # 互联网ETF嘉实
    "159856.SZ",   # 互联网龙头ETF工银
    "159778.SZ",   # 工业互联网ETF鹏华
    "159709.SZ",   # 物联网ETF工银
    "517660.SS",   # 物联网ETF天弘
    "159895.SZ",   # 物联网ETF易方达
    "159896.SZ",   # 物联网ETF南方
    "159701.SZ",   # 物联网ETF招商
    "516330.SS",   # 物联网ETF华泰柏瑞
    "516260.SS",   # 物联网ETF华夏
    "159909.SZ",   # TMT50ETF招商
   
    # ==================== 机器人/智能制造 ====================
    "159542.SZ",   # 工程机械ETF大成
    "159886.SZ",   # 机械ETF富国
    "516960.SS",   # 机械ETF国泰
    "159530.SZ",   # 机器人ETF易方达
    "562500.SS",   # 机器人ETF华夏
    "159770.SZ",   # 机器人ETF天弘
    "159272.SZ",   # 机器人ETF富国
    "159559.SZ",   # 机器人ETF景顺
    "159278.SZ",   # 机器人ETF鹏华
    "159526.SZ",   # 机器人ETF嘉实
    "560770.SS",   # 机器人ETF招商
    "159213.SZ",   # 机器人ETF汇添富
    "159551.SZ",   # 机器人ETF国泰
    "159258.SZ",   # 机器人ETF南方
    "562360.SS",   # 机器人ETF银华
    "560630.SS",   # 机器人ETF万家
    "159667.SZ",   # 工业母机ETF国泰
    "159663.SZ",   # 机床ETF华夏
    "516800.SS",   # 智能制造ETF华宝
    "562910.SS",   # 高端制造ETF易方达
    
    # ==================== 军工/航空航天 ====================
    "159267.SZ",   # 航天ETF华安
    "159257.SZ",   # 航空ETF汇添富
    "159231.SZ",   # 通用航空ETF华宝
    "159392.SZ",   # 航空ETF富国
    "159255.SZ",   # 通用航空ETF易方达
    "516320.SS",   # 高端装备ETF华夏
    "512710.SS",   # 军工龙头ETF富国
    "512660.SS",   # 军工ETF国泰
    "512560.SS",   # 军工ETF易方达
    "512680.SS",   # 军工ETF广发
    "512810.SS",   # 军工ETF华宝
    "512670.SS",   # 国防ETF鹏华
    "159227.SZ",   # 航空航天ETF华夏
    "159241.SZ",   # 航空航天ETF天弘
    "159208.SZ",   # 航空航天ETF万家
    "159638.SZ",   # 高端装备ETF嘉实
    "159378.SZ",   # 通用航空ETF永赢
    "159230.SZ",   # 通用航空ETF华夏
    "159283.SZ",   # 通用航空ETF南方
    "563320.SS",   # 通用航空ETF华泰柏瑞
    "159206.SZ",   # 卫星ETF永赢
    "563230.SS",   # 卫星ETF富国
    "159218.SZ",   # 卫星ETF招商
    
    # ==================== 游戏/传媒 ====================
    "159855.SZ",   # 影视ETF银华
    "517770.SS",   # 游戏传媒ETF浦银
    "159869.SZ",   # 游戏ETF华夏
    "516010.SS",   # 游戏ETF国泰
    "516770.SS",   # 游戏ETF华泰柏瑞
    "512980.SS",   # 传媒ETF广发
    "516190.SS",   # 传媒ETF华夏
    "159805.SZ",   # 传媒ETF鹏华
    "516620.SS",   # 影视ETF国泰
    "159786.SZ",   # VRETF银华
    
    # ==================== 房地产/基建 ====================
    "159787.SZ",   # 建材ETF易方达
    "516750.SS",   # 建材ETF富国
    "159619.SZ",   # 基建ETF国泰
    "512200.SS",   # 房地产ETF南方
    "515060.SS",   # 房地产ETF华夏
    "159707.SZ",   # 地产ETF华宝
    "159768.SZ",   # 房地产ETF银华
    "516950.SS",   # 基建ETF银华
    "516970.SS",   # 基建ETF广发
    "159635.SZ",   # 基建ETF华夏
    "159745.SZ",   # 建材ETF国泰
    "560280.SS",   # 工程机械ETF广发
    
    # ==================== 石油/能源/煤炭 ====================
    "561260.SS",   # 能源ETF工银
    "561790.SS",   # 央企能源ETF博时
    "563150.SS",   # 油气ETF银华
    "159731.SZ",   # 石化ETF华夏
    "562010.SS",   # 绿色能源ETF华宝
    "561360.SS",   # 石油ETF国泰
    "159697.SZ",   # 石油ETF鹏华
    "159588.SZ",   # 石油ETF景顺
    "159148.SZ",   # 石油ETF富国
    "561570.SS",   # 油气ETF华泰柏瑞
    "561760.SS",   # 油气ETF博时
    "159309.SZ",   # 油气ETF汇添富
    "159930.SZ",   # 能源ETF汇添富
    "159945.SZ",   # 能源ETF广发
    "515220.SS",   # 煤炭ETF国泰
    "510170.SS",   # 大宗商品ETF国联安
    "510410.SS",   # 资源ETF博时
    
    # ==================== 黄金/黄金股 ====================
    "159322.SZ",   # 黄金股ETF平安
    "517520.SS",   # 黄金股ETF永赢
    "159562.SZ",   # 黄金股ETF华夏
    "517400.SS",   # 黄金股ETF国泰
    "159315.SZ",   # 黄金股ETF工银
    "159321.SZ",   # 黄金股ETF华安
    
    # ==================== 旅游 ====================
    "159766.SZ",   # 旅游ETF富国
    "562510.SS",   # 旅游ETF华夏
    
    # ==================== 农业/养殖 ====================
    "159275.SZ",   # 农牧渔ETF华宝
    "562900.SS",   # 农业ETF易方达
    "516760.SS",   # 养殖ETF平安
    "159827.SZ",   # 农业ETF银华
    "159616.SZ",   # 农牧ETF建信
    "516550.SS",   # 农业ETF嘉实
    "516810.SS",   # 农业ETF华夏
    "159825.SZ",   # 农业ETF富国
    "516670.SS",   # 畜牧养殖ETF招商
    "159698.SZ",   # 粮食ETF鹏华
    "159587.SZ",   # 粮食ETF广发
    
    # ==================== 物流/运输 ====================
    "159662.SZ",   # 交运ETF南方
    "561320.SS",   # 交运ETF国泰
    "516530.SS",   # 物流ETF银华
    "516910.SS",   # 物流ETF富国
    "159666.SZ",   # 交通运输ETF华夏
    
    # ==================== 教育 ====================
    "513360.SS",   # 教育ETF博时
    
    # ==================== 风格ETF（价值/成长/质量/基本面） ====================
    "159391.SZ",   # 大盘价值ETF博时
    "510030.SS",   # 价值ETF华宝
    "159263.SZ",   # 价值ETF易方达
    "512040.SS",   # 价值100ETF富国
    "159617.SZ",   # 中证500价值ETF华夏
    "159203.SZ",   # 大盘成长ETF博时
    "159259.SZ",   # 成长ETF易方达
    "159906.SZ",   # 深成长ETF大成
    "515910.SS",   # 质量ETF中金
    "512750.SS",   # 基本面50ETF嘉实
    "159910.SZ",   # 基本面120ETF嘉实
    "561500.SS",   # 漂亮50ETF华泰柏瑞
    
    # ==================== 高股息/红利 ====================
    "159207.SZ",   # 高股息ETF广发
    "563180.SS",   # 高股息ETF银华
    
    # ==================== 现金流/自由现金流 ====================
    "561870.SS",   # 现金流全指ETF华富
    "563770.SS",   # 全指现金流ETF招商
    "159236.SZ",   # 自由现金流ETF工银
    "563830.SS",   # 全指现金流ETF博时
    "563680.SS",   # 800现金流ETF汇添富
    "512130.SS",   # 全指现金流ETF鹏华
    "561080.SS",   # 全指现金流ETF华安
    "563620.SS",   # 全指现金流ETF兴业
    "159399.SZ",   # 现金流ETF国泰
    "159232.SZ",   # 自由现金流ETF南方
    "159201.SZ",   # 自由现金流ETF华夏
    "159222.SZ",   # 自由现金流ETF易方达
    "159233.SZ",   # 自由现金流ETF平安
    "159229.SZ",   # 自由现金流ETF广发
    "159223.SZ",   # 现金流ETF永赢
    "159225.SZ",   # 现金流ETF银华
    "159221.SZ",   # 现金流ETF嘉实
    "159276.SZ",   # 现金流ETF汇添富
    "563760.SS",   # 全指现金流ETF中银
    "563390.SS",   # 全指现金流ETF华泰柏瑞
    "563580.SS",   # 自由现金流800ETF万家
    "563780.SS",   # 现金流全指ETF方正富邦
    "563900.SS",   # 300自由现金流ETF摩根
    "562080.SS",   # 300现金流ETF华宝
    "563990.SS",   # 800现金流ETF富国
    "560120.SS",   # 中证500现金流ETF华夏
    "516460.SS",   # 现金流ETF800鹏华
    "159235.SZ",   # 中证现金流ETF大成
    
    # ==================== MSCI系列 ====================
    "515770.SS",   # MSCI中国A股ETF摩根
    "512180.SS",   # MSCIA股ETF建信
    "512360.SS",   # MSCIA股ETF平安
    "512380.SS",   # MSCI中国ETF银华
    "512160.SS",   # MSCI中国A股ETF南方
    "512520.SS",   # MSCI中国ETF华泰柏瑞
    "515160.SS",   # MSCI中国ETF招商
    "512090.SS",   # MSCIA股ETF易方达
    "512990.SS",   # MSCIA股ETF华夏
    
    # ==================== A50/A500系列 ====================
    "563600.SS",   # A500增强ETF易方达
    "512370.SS",   # A500增强ETF华夏
    "563630.SS",   # A500增强ETF国联安
    "563280.SS",   # A50增强ETF富国
    "159240.SZ",   # A500增强ETF天弘
    "561750.SS",   # A50ETF博时
    "512030.SS",   # 中证A50增强ETF易方达
    "512150.SS",   # 富时A50ETF汇安
    "512550.SS",   # 富时A50ETF嘉实
    "159601.SZ",   # A50ETF华夏
    "563000.SS",   # 中国A50ETF易方达
    "159602.SZ",   # 中国A50ETF南方
    "560050.SS",   # 中国A50ETF汇添富
    "561090.SS",   # A500增强ETF华安
    "159226.SZ",   # 中证A500增强ETF国泰
    "159249.SZ",   # A500增强ETF工银
    
    # ==================== 央企/国企 ====================
    "560810.SS",   # 央企ESGETF融通
    "563060.SS",   # 央企50ETF易方达
    "510060.SS",   # 央企ETF工银
    "561960.SS",   # 央企回报ETF招商
    "517090.SS",   # 央企共赢ETF国泰
    "515900.SS",   # 央企创新ETF博时
    "515680.SS",   # 央企创新ETF嘉实
    "515600.SS",   # 央企创新ETF广发
    "159974.SZ",   # 央企创新ETF富国
    "512960.SS",   # 央企结构调整ETF博时
    "159959.SZ",   # 央企ETF银华
    "562380.SS",   # 央企科技ETF银华
    "563050.SS",   # 央企科技ETF易方达
    "517180.SS",   # 中国国企ETF南方
    "159719.SZ",   # 国企ETF平安
    "510810.SS",   # 上海国企ETF汇添富
    "510270.SS",   # 国企ETF中银
    "159528.SZ",   # 国企改革ETF富国
    "159335.SZ",   # 央企科创ETF融通
    "560170.SS",   # 央企科技ETF南方
    "512950.SS",   # 央企改革ETF华夏
    "562850.SS",   # 央企能源ETF嘉实
    "515760.SS",   # 浙江国资ETF华夏
    
    # ==================== 科创系列 ====================
    "589580.SS",   # 科创综指ETF兴银
    "589050.SS",   # 科创综指ETF兴业
    "589550.SS",   # 科创价值ETF华夏
    "589200.SS",   # 科创200ETF工银
    "588520.SS",   # 科创增强ETF永赢
    "589820.SS",   # 科创200ETF建信
    "589780.SS",   # 科创200ETF富国
    "589060.SS",   # 科创综指ETF东财
    "588690.SS",   # 科创增强ETF银华
    "588680.SS",   # 科创100增强ETF广发
    "588880.SS",   # 科创100ETF华泰柏瑞
    "588550.SS",   # 科创综指增强ETF易方达
    "588980.SS",   # 科创100ETF广发
    "589980.SS",   # 科创100ETF汇添富
    "589950.SS",   # 科创100ETF富国
    "589890.SS",   # 科创综指ETF景顺
    "588670.SS",   # 科创综指增强ETF嘉实
    "589700.SS",   # 科创成长ETF南方
    "588070.SS",   # 科创成长ETF万家
    "588800.SS",   # 科创100ETF华夏
    "588210.SS",   # 科创100ETF易方达
    "588020.SS",   # 科创成长ETF易方达
    "588110.SS",   # 科创成长ETF广发
    "588010.SS",   # 科创新材料ETF博时
    "588160.SS",   # 科创新材料ETF南方
    "589180.SS",   # 科创新材料ETF汇添富
    "588230.SS",   # 科创200ETF华泰柏瑞
    "588240.SS",   # 科创200ETF鹏华
    "588820.SS",   # 科创200ETF华夏
    "588270.SS",   # 科创200ETF易方达
    "588140.SS",   # 科创200ETF广发
    "588850.SS",   # 科创机械ETF嘉实
    "588910.SS",   # 科创价值ETF建信
    "589500.SS",   # 科创综指ETF工银
    "589600.SS",   # 科创综指ETF富国
    "589900.SS",   # 科创综指ETF博时
    
    # ==================== ESG/碳中和 ====================
    "510990.SS",   # 180ESGETF工银
    "159653.SZ",   # ESG300ETF国联安
    "159621.SZ",   # ESGETF国泰
    "159717.SZ",   # ESGETF鹏华
    "516720.SS",   # ESGETF浦银
    "560180.SS",   # 沪深300ESGETF南方
    "561190.SS",   # 碳中和ETF富国
    "560550.SS",   # 碳中和ETF广发
    "159639.SZ",   # 碳中和ETF南方
    "159640.SZ",   # 碳中和龙头ETF工银
    "560060.SS",   # 碳中和ETF汇添富
    "159641.SZ",   # 碳中和ETF招商
    "562990.SS",   # 碳中和ETF易方达
    "159642.SZ",   # 碳中和ETF大成
    "159861.SZ",   # 碳中和50ETF国泰
    "562300.SS",   # 碳中和ETF银华
    "159885.SZ",   # 碳中和ETF鹏华
    "560560.SS",   # 碳中和ETF泰康
    "515090.SS",   # 可持续发展ETF博时
    "516070.SS",   # 低碳ETF易方达
    
    # ==================== 区域主题 ====================
    "512870.SS",   # 杭州湾区ETF南华
    "159743.SZ",   # 湖北ETF博时
    "517330.SS",   # 长江保护ETF易方达
    "512190.SS",   # 浙商之江凤凰ETF
    "159976.SZ",   # 湾创ETF工银
    "512970.SS",   # 大湾区ETF平安
    "517160.SS",   # 长江保护ETF南方
    "512650.SS",   # 长三角ETF汇添富
    "510770.SS",   # G60创新ETF申万菱信
    "517850.SS",   # 张江ETF汇添富
    "159623.SZ",   # 成渝经济圈ETF博时
    
    # ==================== 指数/策略ETF ====================
    "510010.SS",   # 180治理ETF交银
    "510090.SS",   # 责任ETF建信
    "159965.SZ",   # 央视50ETF国联
    "159578.SZ",   # 深证主板50ETF南方
    "563330.SS",   # A股ETF华泰柏瑞
    "530530.SS",   # 上证580ETF华夏
    "159290.SZ",   # 创业板综指增强ETF东财
    "159291.SZ",   # 创业板综增强ETF招商
    "159287.SZ",   # 创业板综ETF博时
    "159288.SZ",   # 创业板综ETF银华
    "159289.SZ",   # 创业板综指ETF鹏华
    "515200.SS",   # 创新100ETF申万菱信
    "512770.SS",   # 战略新兴ETF华夏
    "159966.SZ",   # 创业板价值ETF华夏
    "510160.SS",   # 产业升级ETF南方
    
    # ==================== 其他主题 ====================
    "516560.SS",   # 养老ETF华宝
    "159973.SZ",   # 民企ETF弘毅远方
    "515150.SS",   # 一带一路ETF富国
    "515110.SS",   # 一带一路ETF易方达
    "515990.SS",   # 一带一路ETF汇添富
    "159597.SZ",   # 创业板成长ETF易方达
]

# ==================== ⏱️ 物理时间监控装饰器 ====================
def time_monitor(func_name=None):
    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            start_time = time.time()
            start_real = datetime.datetime.now()
            log.info('⏱️ [{}] 开始执行 - 真实时间: {}'.format(func_name or func.__name__, start_real.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]))
            try:
                result = func(*args, **kwargs)
                end_time = time.time()
                end_real = datetime.datetime.now()
                elapsed = end_time - start_time
                log.info('✅ [{}] 执行完成 - 耗时: {:.2f}ms | 真实时间: {}'.format(func_name or func.__name__, elapsed * 1000, end_real.strftime('%H:%M:%S')))
                if elapsed > 2.0:
                    log.warning('⚠️ [{}] 执行耗时过长: {:.2f}ms'.format(func_name or func.__name__, elapsed * 1000))
                return result
            except Exception as e:
                end_time = time.time()
                elapsed = end_time - start_time
                log.error('❌ [{}] 执行异常 - 耗时: {:.2f}ms | 错误: {}'.format(func_name or func.__name__, elapsed * 1000, str(e)[:100]))
                raise
        return wrapper
    return decorator

# ==================== 📐 中英文混排对齐工具 ====================
def get_display_width(s):
    width = 0
    for ch in str(s):
        if '\u4e00' <= ch <= '\u9fff' or '\u3000' <= ch <= '\u303f' or '\uff00' <= ch <= '\uffef':
            width += 2
        else:
            width += 1
    return width

def pad_to_width(s, target_width):
    s = str(s)
    current = get_display_width(s)
    if current >= target_width:
        return s
    return s + ' ' * (target_width - current)

def format_table_row(columns, widths):
    return ' '.join(pad_to_width(col, w) for col, w in zip(columns, widths))


# ==================== 初始化模块 ====================
def initialize(context):
    # ---------- 交易设置 ----------
    if not is_trade():  # 费率/滑点设置为回测专用，实盘由券商柜台生效（原 JQ）
        set_slippage(0.0001)  # 百分比滑点（原 JQ PriceRelatedSlippage）
    if not is_trade():  # 回测专用费率（实盘由柜台生效），原 JQ set_order_cost/OrderCost
        set_commission(commission_ratio=0.0001, min_commission=5, type="ETF")
    set_benchmark("161226.SZ")
    log.info("🚀================ 策略初始化开始 ================🚀")

    # ---------- ETF池 ----------
    g.etf_pool_bak = [
        "518880.SS",   # 黄金ETF
        "159985.SZ",   # 豆粕ETF
        "501018.SS",   # 南方原油
        "161226.SZ",   # 白银LOF
        "513100.SS",   # 纳指ETF
        "159915.SZ",   # 创业板ETF
        "511220.SS",   # 城投债ETF
    ]
    g.etf_pool = MASTER_ETF_POOL.copy()
    g.industry_etf_pool = INDUSTRY_ETF_POOL
    g.all_etf_pool = list(set(g.etf_pool + g.industry_etf_pool))
    
    g.seesaw_etf_pool = [
        # 银行
        "512800.SS", "516310.SS", "515020.SS", "159887.SZ",
        "512820.SS", "512700.SS", "515290.SS", "512730.SS",
        "516210.SS", "517900.SS",
        # 电力/公用事业
        "560620.SS", "560190.SS", "159669.SZ", "512580.SS",
        "159611.SZ", "561560.SS", "562550.SS", "562960.SS",
        "561170.SS", "159625.SZ", "562350.SS", "561700.SS",
        "560580.SS", "159301.SZ", "159320.SZ", "561380.SS",
        "159326.SZ",
        # 红利
        "159336.SZ", "159332.SZ", "512390.SS", "562060.SS",
        "561680.SS", "560700.SS", "512530.SS",
        # 高股息
        "159207.SZ", "563180.SS",
        # 央企/国企
        "560810.SS", "563060.SS", "510060.SS", "561960.SS",
        "517090.SS", "515900.SS", "515680.SS", "515600.SS",
        "159974.SZ", "512960.SS", "159959.SZ", "562380.SS",
        "563050.SS", "517180.SS", "159719.SZ", "510810.SS",
        "510270.SS", "159528.SZ", "159335.SZ", "560170.SS",
        "512950.SS", "562850.SS", "515760.SS",
        # 黄金/黄金股（避险资产）
        "159322.SZ", "517520.SS", "159562.SZ", "517400.SS",
        "159315.SZ", "159321.SZ",
        # 石油/能源/煤炭（周期资源）
        "561260.SS", "159930.SZ", "159945.SZ", "515220.SS",
        "561790.SS", "561360.SS",
        # 农业/养殖（防御属性）
        "516770.SS", "159855.SZ",
    ]
    
    # ---------- 动态扩容池参数 ----------
    g.dynamic_etf_pool = {}
    g.broadcast_history = []
    g.dynamic_pool_days = 10
    g.dynamic_pool_entry_days = 2

    # ---------- 核心参数 ----------
    g.lookback_days = 25
    g.holdings_num = 1
    g.min_money = 5000

    # ---------- 盈利保护参数 ----------
    g.enable_profit_protection = True
    g.profit_protection_lookback = 1
    g.profit_protection_threshold = 0.05
    g.profit_protection_check_times = ['11:00']

    g.loss = 0.97
    g.min_score_threshold = 0
    g.max_score_threshold = 100.0

    # ---------- 成交量过滤 ----------
    g.enable_volume_check = True
    g.volume_lookback = 5
    g.volume_threshold = 2
    g.volume_return_limit = 1

    # ---------- 短期动量过滤 ----------
    g.use_short_momentum_filter = True
    g.short_lookback_days = 10
    g.short_momentum_threshold = 0.0

    # ---------- 溢价率过滤 ----------
    g.enable_premium_filter = True
    g.premium_threshold = 0.20

    # ---------- 运行时变量 ----------
    g.rankings_cache = {'date': None, 'data': None}
    g.trade_log = {'records': [], 'sell_records': []}
    g.prev_total_value = None
    g.initial_cash = None  

    # ---------- 震荡期参数 ----------
    g.enable_range_bound_mode = True
    g.current_filter = '正常期'
    g.risk_state = '正常期'
    g.lookback_high_low_days = 20
    g.risk_benchmark = '510300.SS'
    g.laplace_s_param = 0.05
    g.laplace_min_slope = 0.001
    g.gaussian_sigma = 1.2
    g.gaussian_min_slope = 0.002
    g.enable_bias_trigger = True
    g.bias_threshold = 0.10
    g.ma_period = 20
    g.enable_rsi_trigger = True
    g.rsi_overbought = 75
    g.rsi_pullback = 60
    g.previous_rsi = None
    g.enable_stop_loss_trigger = False
    g.stop_loss_triggered_today = False
    g.stop_loss_triggered_date = None
    g.enable_low_point_rise_trigger = True
    g.low_point_rise_threshold = 0.03
    g.enable_stable_signal_trigger = True
    g.drawdown_recovery = 0.03
    g.max_range_bound_days = 15
    g.stable_days = 0
    g.filter_switch_cooldown = 2
    g.last_switch_date = None
    g.range_bound_start_date = None
    g.range_bound_days_count = 0
    g.previous_drawdown = None

    # ---------- 龙头家族变量 ----------
    g.dragon_head_etf = None
    g.dragon_head_name = None
    g.dragon_family = {}

    # ---------- 交易调度 ----------
    run_daily(context, get_cached_rankings, time='13:19')
    run_daily(context, etf_sell_trade, time='13:20')
    run_daily(context, etf_buy_trade, time='13:21')

    for check_time in g.profit_protection_check_times:
        run_daily(context, profit_protection_check, time=check_time)
        log.info('已注册盈利保护检查时间：{}'.format(check_time))

    run_daily(context, check_range_bound, time='13:05')

    # ---------- 动态池调度 ----------
    run_daily(context, check_dynamic_pool_entry, time='15:00')
    log.info("✅ 已注册动态池管理时间：09:05(生命周期管理), 15:00(准入检查)")

    # ---------- 播报调度 ----------
    run_daily(context, run_broadcasts, time='10:30')
    run_daily(context, run_broadcasts, time='14:50')
    log.info("✅ 已注册全市场ETF播报时间：10:30, 14:50（5日+10日双周期）")
    log.info("✅ 已注册盘后龙头监测时间：15:01")

    # ---------- 盘后总结 ----------
    log.info("✅ 已注册盘后总结时间：15:05")

    # 首次运行，判断震荡期
    init_range_bound_status(context)

    # ---------- 打印初始化清单 ----------
    print_strategy_config(context)

    log.info("========== 策略初始化完成 ==========")
    log.info('交易ETF池{}只，行业ETF池{}只，播报全池{}只'.format(len(g.etf_pool), len(g.industry_etf_pool), len(g.all_etf_pool)))

# ==================== 📋 策略初始化清单打印 ====================
def print_strategy_config(context):
    log.info("=" * 70)
    log.info("📋 策略配置清单 [ 🚀七星1.72+行业全市场播报 🚀 ]")
    log.info("=" * 70)
    log.info("📌 ETF池配置:")
    log.info('  交易主ETF池数量: {} 只'.format(len(g.etf_pool)))
    log.info('  行业ETF池数量（播报用）: {} 只'.format(len(g.industry_etf_pool)))
    log.info('  播报全市场ETF池: {} 只（主池+行业池去重）'.format(len(g.all_etf_pool)))
    log.info('  动态扩容池: {} 只'.format(len(g.dynamic_etf_pool)))
    for etf, days in g.dynamic_etf_pool.items():
        log.info('    {} {} - 剩余{}天'.format(etf, get_name(etf), days))
    log.info("📌 核心参数:")
    log.info('  动量计算周期: {} 天'.format(g.lookback_days))
    log.info('  最大持仓数量: {} 只'.format(g.holdings_num))
    log.info('  最小交易金额: {} 元'.format(g.min_money))
    log.info('  得分范围: [{}, {}]'.format(g.min_score_threshold, g.max_score_threshold))
    log.info("📌 过滤条件:")
    log.info('  盈利保护: {}'.format('✅ 开启' if g.enable_profit_protection else '❌ 关闭'))
    if g.enable_profit_protection:
        log.info('    回看天数: {} 天'.format(g.profit_protection_lookback))
        log.info('    回撤阈值: {:.0f}%'.format(g.profit_protection_threshold * 100))
        log.info('    检查时间: {}'.format(', '.join(g.profit_protection_check_times)))
    log.info('  溢价率过滤: {}'.format('✅ 开启' if g.enable_premium_filter else '❌ 关闭'))
    if g.enable_premium_filter:
        log.info('    溢价率阈值: {:.0f}%'.format(g.premium_threshold * 100))
    log.info('  成交量过滤: {}'.format('✅ 开启' if g.enable_volume_check else '❌ 关闭'))
    if g.enable_volume_check:
        log.info('    回看天数: {} 天'.format(g.volume_lookback))
        log.info('    量比阈值: {} 倍'.format(g.volume_threshold))
        log.info('    收益触发线: {:.0f}%'.format(g.volume_return_limit * 100))
    log.info('  短期动量过滤: {}'.format('✅ 开启' if g.use_short_momentum_filter else '❌ 关闭'))
    if g.use_short_momentum_filter:
        log.info('    回看天数: {} 天'.format(g.short_lookback_days))
        log.info('    阈值: {:.0f}%'.format(g.short_momentum_threshold * 100))
    log.info('  近3日跌幅过滤: 阈值 {:.0f}%'.format(g.loss * 100))
    log.info("📌 震荡期机制:")
    log.info('  功能开关: {}'.format('✅ 开启' if g.enable_range_bound_mode else '❌ 关闭'))
    if g.enable_range_bound_mode:
        log.info('    当前状态: {} (滤波器: {})'.format(g.risk_state, g.current_filter))
        log.info('    高低点回看: {} 天'.format(g.lookback_high_low_days))
        log.info('    切换冷却期: {} 个交易日'.format(g.filter_switch_cooldown))
        log.info('    最大震荡天数: {} 天'.format(g.max_range_bound_days))
        log.info('   【拉普拉斯参数】 s: {}, min_slope: {}'.format(g.laplace_s_param, g.laplace_min_slope))
        log.info('   【高斯参数】 sigma: {}, min_slope: {}'.format(g.gaussian_sigma, g.gaussian_min_slope))
        log.info('   【进入条件】 乖离率触发: {} 阈值 {:.0f}%'.format('✅' if g.enable_bias_trigger else '❌', g.bias_threshold * 100))
        log.info('               RSI超买回落: {} 超买 {} 回落 {}'.format('✅' if g.enable_rsi_trigger else '❌', g.rsi_overbought, g.rsi_pullback))
        log.info('               盈利保护触发: {}'.format('✅' if g.enable_stop_loss_trigger else '❌'))
        log.info('   【退出条件】 低点上涨退出: {} 阈值 {:.0f}%'.format('✅' if g.enable_low_point_rise_trigger else '❌', g.low_point_rise_threshold * 100))
        log.info('               稳定信号退出: {} 回撤收窄 {:.0f}%'.format('✅' if g.enable_stable_signal_trigger else '❌', g.drawdown_recovery * 100))
    log.info("📌 交易调度:")
    log.info('  09:00 - pre_market_correlation_analysis（📊 开盘前相关性分析）'.format())
    log.info('  09:10 - check_positions（持仓检查）'.format())
    log.info('  10:30 - run_broadcasts（📢 全市场ETF播报 7日+10日）'.format())
    log.info('  11:00 - profit_protection_check（盈利保护检查）'.format())
    log.info('  13:10 - get_cached_rankings（计算ETF排名）'.format())
    log.info('  13:20 - etf_sell_trade（卖出操作）'.format())
    log.info('  13:21 - etf_buy_trade（买入操作）'.format())
    log.info('  13:05 - check_range_bound（震荡期检查）'.format())
    log.info('  14:50 - run_broadcasts（📢 全市场ETF播报 7日+10日）'.format())
    log.info('  15:05 - daily_summary（📊 盘后总结）'.format())
    log.info('  15:10 - reset_range_bound_daily（震荡期重置）'.format())
    log.info("📌 当前运行状态:")
    log.info('  震荡期: {}'.format(g.risk_state))
    log.info("✅ 配置清单打印完成")
    log.info("=" * 70)

# ==================== 🏷️ 主题分类工具 ====================
def get_theme(name):
    if not name:
        return '其他'
    theme_keywords = {
        '芯片半导体': ['芯片','半导体','集成电路','科创芯片','芯片设计','半导体设备'],
        '科创': ['科创','科创板','科创50','科创新'],
        '红利': ['红利','股息','高股息'],
        '电力': ['电力','绿电','公用事业','电网'],
        '消费': ['消费','食品饮料','家电','酒','消费电子'],
        '医药': ['医药','医疗','生物','创新药','中药'],
        '通信': ['通信','5G','电信'],
        '证券': ['证券','券商','保险'],
        '银行': ['银行','金融'],
        '新能源': ['新能源','光伏','电池','碳中和','储能'],
        '有色': ['有色','稀土','稀有金属','矿业','黄金'],
        '化工': ['化工','石化','能源化工'],
        '人工智能': ['人工智能','AI','云计算','软件','大数据','信创'],
        '机器人': ['机器人','智能制造','工业母机','机床'],
        '军工': ['军工','国防','航空航天','卫星','通用航空'],
        '石油': ['石油','油气','能源','煤炭'],
        '旅游': ['旅游','出行'],
        '农业': ['农业','养殖','畜牧','粮食','豆粕'],
        '物流': ['物流','运输'],
        '教育': ['教育'],
        '成长': ['成长'],
        '价值': ['价值'],
        '跨境': ['跨境','海外','港股','纳指','标普','恒生','日经','德国','法国','东南亚','中韩'],
        '债券': ['债券','国债','城投','可转债'],
        '地产': ['房地产','地产','基建','建材'],
        '传媒': ['传媒','游戏','影视','VR'],
        '汽车': ['汽车','智能驾驶','汽车零部件'],
        '创业板': ['创业板'],
        '中证': ['中证'],
        '沪深': ['沪深'],
        'A500': ['A500'],
    }
    for theme, keywords in theme_keywords.items():
        for kw in keywords:
            if kw in name:
                return theme
    return '其他'

def compress_etfs_by_theme(etf_list, score_key='cum_score', max_per_theme=6):
    groups = {}
    for item in etf_list:
        theme = get_theme(item['name'])
        groups.setdefault(theme, []).append(item)
    compressed = []
    for theme, items in groups.items():
        items_sorted = sorted(items, key=lambda x: x.get(score_key, 0), reverse=True)
        compressed.extend(items_sorted[:max_per_theme])
    compressed.sort(key=lambda x: x.get(score_key, 0), reverse=True)
    return compressed

def interleave_by_theme(compressed_list, max_count=20):
    theme_groups = {}
    for item in compressed_list:
        theme = get_theme(item['name'])
        theme_groups.setdefault(theme, []).append(item)
    for theme, items in theme_groups.items():
        score_key = 'composite_score' if 'composite_score' in items[0] else 'cum_score'
        items.sort(key=lambda x: x.get(score_key, 0), reverse=True)
    theme_order = sorted(theme_groups.keys(),
                         key=lambda t: theme_groups[t][0].get('composite_score', theme_groups[t][0].get('cum_score', 0)),
                         reverse=True)
    result = []
    indices = {theme: 0 for theme in theme_groups}
    while len(result) < max_count:
        any_added = False
        for theme in theme_order:
            if len(result) >= max_count:
                break
            idx = indices[theme]
            if idx < len(theme_groups[theme]):
                result.append(theme_groups[theme][idx])
                indices[theme] += 1
                any_added = True
        if not any_added:
            break
    return result

# ==================== 📢 全市场ETF播报函数 ====================
def market_broadcast(context, days=5):
    """遍历主池+行业池，按主题压缩+轮询展示前20名"""
    log.info("=" * 70)
    log.info('📢 全市场ETF行情播报（{}日周期）- {}'.format(days, context.current_dt.strftime('%Y-%m-%d %H:%M')))
    log.info("=" * 70)

    etf_performance = []
    all_etfs = g.all_etf_pool
    for etf in all_etfs:
        try:
            if get_current_data()[etf].paused:
                continue
            hist = jq_attribute_history(etf, max(days, 7), '1d', ['close', 'high', 'low', 'open'])
            if len(hist) < days:
                continue
            
            closes = hist['close'].tolist()
            highs = hist['high'].tolist()
            lows = hist['low'].tolist()
            opens = hist['open'].tolist()
            
            # 当日最新价取自上一分钟已收盘K线（jq_last_bar，fq='pre' 同复权链），
            # 不再用未复权 last_price，避免 fq='pre' 历史价混尺度导致涨幅爆炸（且无未来函数）。
            live_df = jq_last_bar(etf, '1m', ['close', 'high', 'low'])
            if live_df is None or live_df.empty:
                continue
            try:
                _live_close = float(live_df['close'].iloc[-1])
                _live_high = float(live_df['high'].iloc[-1])
                _live_low = float(live_df['low'].iloc[-1])
            except Exception:
                continue
            if not _live_close or _live_close <= 0:
                continue
            prev_close = closes[-1] if closes else 0
            # 兜底校验：当日K线与前收比例异常（真停牌/数据错位）则弃用实时价
            if prev_close > 0 and not (0.5 < _live_close / prev_close < 2.0):
                log.debug('⚠️ {} {} 当日价{:.3f}与前收{:.3f}比例{:.2f}x异常，播报弃用当日价'.format(
                    etf, get_name(etf), _live_close, prev_close, _live_close / prev_close))
            else:
                closes.append(_live_close)
                highs.append(_live_high)
                lows.append(_live_low)
            
            pos_days = sum(1 for i in range(1, len(closes)) if closes[i] > closes[i-1])
            neg_days = len(closes) - 1 - pos_days
            cum_score = closes[-1] / closes[0] - 1
            
            alpha003 = calculate_alpha003(closes, lows, highs)
            
            # 检查是否有连续三日高开（开盘价 > 前一日收盘价）
            has_consecutive_high_open = False
            for i in range(2, len(opens)):
                if opens[i] > closes[i-1] and opens[i-1] > closes[i-2] and opens[i-2] > closes[i-3]:
                    has_consecutive_high_open = True
                    break
            
            # 🎯 修复：更严格的"优选"标准
            is_preferred = False
            mid = len(closes) // 2
            threshold = 0.06 if days == 10 else 0.03
            if len(closes) >= days + 1 and cum_score > threshold:
                first_half_score = closes[mid] / closes[0] - 1
                second_half_score = closes[-1] / closes[mid] - 1
                first_half_pos = sum(1 for i in range(1, mid + 1) if closes[i] > closes[i-1])
                second_half_pos = sum(1 for i in range(mid + 1, len(closes)) if closes[i] > closes[i-1])
                if second_half_score > first_half_score * 1.5 and second_half_pos >= first_half_pos and second_half_score > 0:
                    is_preferred = True
            elif len(closes) >= 4 and cum_score > threshold:
                if cum_score > 0 and pos_days >= 2:
                    is_preferred = True

            etf_performance.append({
                'etf': etf,
                'name': get_name(etf),
                'cum_score': cum_score,
                'pos_days': pos_days,
                'neg_days': neg_days,
                'alpha003': alpha003,
                'is_preferred': is_preferred,
                'has_consecutive_high_open': has_consecutive_high_open
            })
        except Exception:
            continue

    if not etf_performance:
        log.info("📭 无有效数据")
        log.info("=" * 70)
        return

    for item in etf_performance:
        pos_ratio = item['pos_days'] / float(days)
        item['composite_score'] = item['cum_score'] * (0.6 + 0.4 * pos_ratio)

    alpha003_sorted = sorted(etf_performance, key=lambda x: x['alpha003'], reverse=True)
    alpha003_rank_map = {item['etf']: idx + 1 for idx, item in enumerate(alpha003_sorted)}

    compressed = compress_etfs_by_theme(etf_performance, score_key='composite_score')
    max_count = 10
    final_top20 = interleave_by_theme(compressed, max_count=max_count)

    total = len(etf_performance)
    full_sorted = sorted(etf_performance, key=lambda x: x['composite_score'], reverse=True)
    rank_map = {item['etf']: idx for idx, item in enumerate(full_sorted)}

    star_list = []
    for item in final_top20:
        idx = rank_map.get(item['etf'], total)
        percentile = idx / total if total > 0 else 1
        if percentile < 0.10:
            star = "★★★★★"
        elif percentile < 0.30:
            star = "★★★★"
        elif percentile < 0.50:
            star = "★★★"
        elif percentile < 0.70:
            star = "★★"
        else:
            star = "★"
        star_list.append(star)

    headers = ['排名', 'ETF代码', '名称', '{}日涨幅'.format(days), '上涨天数', '负天数', 'Alpha003', 'A003排名', '星级', '优选', '三连高开', '龙头家族']
    widths = [4, 14, 16, 10, 6, 6, 10, 8, 6, 4, 6, 8]
    log.info(format_table_row(headers, widths))
    log.info("-" * 92)
    
    for i, item in enumerate(final_top20):
        tag = "⭐" if item['is_preferred'] else ""
        
        family_tag = ""
        if g.dragon_head_etf is not None:
            if item['etf'] == g.dragon_head_etf or item['etf'] in g.dragon_family:
                family_tag = "👑龙家"
        
        name_display = item['name'][:10] if len(item['name']) > 10 else item['name']
        alpha003_rank = alpha003_rank_map.get(item['etf'], "-")
        high_open_tag = "↑" if item['has_consecutive_high_open'] else ""
        cols = [
            str(i+1),
            item['etf'],
            name_display,
            '{:>9.2f}%'.format(item['cum_score'] * 100),
            str(item['pos_days']),
            str(item['neg_days']),
            '{:>9.2f}%'.format(item['alpha003'] * 100),
            str(alpha003_rank),
            star_list[i],
            tag,
            high_open_tag,
            family_tag
        ]
        log.info(format_table_row(cols, widths))
    log.info("-" * 88)
    log.info('📊 总计: {} 只ETF | 展示前{}只'.format(total, len(final_top20)))
    log.info("=" * 88)
    
    if context.current_dt.hour >= 14:
        top5_etfs = [item['etf'] for item in final_top20[:5]]
        record_broadcast_history(context, top5_etfs)

def run_broadcasts(context):
    """运行所有独立播报"""
    market_broadcast(context, days=7)
    market_broadcast(context, days=10)

# ==================== 🔄 代码修改后自动刷新 ====================
def after_code_changed(context):
    """
    实盘修改代码后自动刷新ETF池，并按分类展示明细
    """
    log.info("🔄 ========== 检测到代码修改，开始刷新策略配置 ==========")
    
    # 重新加载 ETF 池，保留动态池
    update_etf_pool_with_dynamic()
    g.rankings_cache = {'date': None, 'data': None}
    
    # 使用 getattr 安全获取参数，避免 KeyError
    log.info('📈 盈利保护：{}，回撤{:.0f}%'.format('开' if g.enable_profit_protection else '关', g.profit_protection_threshold * 100))
    
    # ========== 展示动态ETF池 ==========
    log.info("📋 ========== ETF池明细 ==========")
    log.info('📊 主池总计：{} 只标的'.format(len(MASTER_ETF_POOL)))
    
    if g.dynamic_etf_pool:
        log.info("🔄 动态池（{}只）：".format(len(g.dynamic_etf_pool)))
        for etf, days in sorted(g.dynamic_etf_pool.items(), key=lambda x: -x[1]):
            try:
                name = get_name(etf)
            except:
                name = "未知"
            log.info('  {}({}) - 剩余{}天'.format(etf, name, days))
    else:
        log.info("� 动态池：空")
    
    log.info("📋 ========== 展示完毕 ==========")
    log.info("🔄 ========== 配置刷新完成 ==========\n")

# ==================== 开盘持仓检查 ====================
def check_positions(context):
    log.info('\n======================🐂🧨🧨🧨🧨🧨{}📌策略运行开始📌一路长红🧨🧨🧨🧨🧨🐂======================'.format(context.current_dt.strftime('%Y-%m-%d')))
    g.profit_protection_sold_today = []
    g.trade_log['sell_records'] = []
    if g.initial_cash is None:
        g.initial_cash = context.portfolio.portfolio_value
        log.info('📌 记录初始总资产: {:,.2f}'.format(g.initial_cash))
    for sec in context.portfolio.positions:
        pos = context.portfolio.positions[sec]
        if pos.amount > 0:
            log.info('📊 持仓：{} {} 数量{} 成本{:.3f} 现价{:.3f}'.format(sec, get_name(sec), pos.amount, pos.cost_basis, pos.last_sale_price))

# ==================== 🚫 停牌检测函数 ====================
def is_etf_suspended(etf, context):
    """判断ETF是否停牌。返回: True=停牌, False=正常交易。

    PTrade 回测中 get_price(frequency='1m') 经常返回空（分钟数据不可用），
    原 JQ 把"盘中无分钟数据"直接判为停牌，会导致全池被误杀。改为：
      1) 以 get_current_data()[etf].paused 为主信号（回测恒 False，实盘来自 get_stock_status）；
      2) 分钟成交量仅作辅助——仅当分钟数据"确实存在且近期无成交"才判停牌；
      3) 分钟数据整体不可用时保守返回 False，绝不误杀。
    """
    try:
        if get_current_data()[etf].paused:
            log.debug('🚫 {} ({}) 停牌（paused）'.format(etf, get_name(etf)))
            return True
    except Exception:
        pass
    try:
        today = context.current_dt.date()
        minute_data = jq_get_price(
            etf,
            start_date=today,
            end_date=context.current_dt,
            frequency='1m',
            fields=['volume'],
            skip_paused=False,
            panel=False
        )
        name = get_name(etf)

        if minute_data is None or minute_data.empty:
            # 分钟数据不可用（PTrade 回测常见）：无法据此判停牌，保守返回 False
            return False

        minute_count = len(minute_data)
        check_minutes = min(5, minute_count)
        recent_vol = minute_data['volume'].tail(check_minutes).sum()

        if recent_vol == 0:
            log.debug('🚫 {} ({}) 停牌（最近{}分钟无成交）'.format(etf, name, check_minutes))
            return True

        return False
    except Exception as e:
        log.debug('{} 停牌检测异常: {}'.format(etf, e))
        return False

# ==================== 盈利保护 ====================
def profit_protection_check(context):
    if not g.enable_profit_protection:
        return
    log.info("========== 盈利保护独立检查开始 ==========")
    for sec in list(context.portfolio.positions.keys()):
        pos = context.portfolio.positions[sec]
        if pos.amount == 0:
            continue
        
        if sec not in g.etf_pool:
            if smart_order_target_value(sec, 0, context):
                log.info('📤 盈利保护卖出（不在交易池）：{} {}'.format(sec, get_name(sec)))
            continue
        
        if check_profit_protection(sec, context):
            if smart_order_target_value(sec, 0, context):
                log.info('🛡️ 盈利保护卖出（独立检查）：{} {}'.format(sec, get_name(sec)))
                if getattr(g, 'enable_stop_loss_trigger', False):
                    g.stop_loss_triggered_today = True
                    g.stop_loss_triggered_date = context.current_dt.date()
                    log.info("【盈利保护触发】记录止损信号")
    log.info("========== 盈利保护独立检查完成 ==========")

def check_profit_protection(security, context, lookback=None, threshold=None):
    if not g.enable_profit_protection:
        return False
    lookback = lookback or g.profit_protection_lookback
    threshold = threshold or g.profit_protection_threshold
    hist = jq_attribute_history(security, lookback, '1d', ['high'])
    if hist.empty or len(hist) < lookback:
        return False
    max_high = hist['high'].max()
    current_price = get_current_data()[security].last_price
    if current_price <= max_high * (1 - threshold):
        log.info('🔻 {} {} 触发盈利保护：回撤{:.2f}%'.format(security, get_name(security), (1 - current_price / max_high) * 100))
        return True
    return False

def get_premium_rate(code, date, max_back_days=5):
    """获取 ETF 溢价率（现价 vs 净值）。

    实盘计算，回测跳过（回测环境中 get_snapshot/get_etf_info 可能不可用）。
    """
    # 回测跳过溢价率过滤
    if not is_trade():
        return None, None, None

    try:
        price_data = get_history(count=max_back_days, frequency='1d',
                                 field=['close'], security_list=[code],
                                 fq='pre', include=True, is_dict=False)
        if price_data is None or price_data.empty:
            log.debug('{} 无交易价格数据'.format(code))
            return None, None, None
        price = float(price_data['close'].iloc[-1])
    except Exception:
        log.debug('{} 取价失败'.format(code))
        return None, None, None

    net_value = None
    # 优先盘中 IOPV
    try:
        snap = get_snapshot(code)
        if snap and code in snap:
            net_value = snap[code].get('iopv')
    except Exception:
        pass

    # 退路：前一日净值
    if net_value is None or net_value <= 0:
        try:
            etf_info = get_etf_info(code)
            if etf_info and code in etf_info:
                net_value = etf_info[code].get('nav_pre')
        except Exception:
            pass

    if net_value is None or net_value <= 0:
        log.debug('{} 无净值数据'.format(code))
        return None, None, None

    premium_rate = (price - net_value) / net_value
    log.info('=== {} 现价:{:.4f} 净值:{:.4f} ==='.format(code, price, net_value))
    return premium_rate, price, net_value

# ==================== 震荡期机制 ====================
def calculate_rsi(close, period=14):
    try:
        if len(close) < period + 1:
            return None
        deltas = np.diff(close)
        gains = np.where(deltas > 0, deltas, 0)
        losses = np.where(deltas < 0, -deltas, 0)
        avg_gain = np.mean(gains[-period:])
        avg_loss = np.mean(losses[-period:])
        if avg_loss == 0:
            return 100
        rs = avg_gain / avg_loss
        rsi = 100 - (100 / (1 + rs))
        return rsi
    except:
        return None

def laplace_filter(price, s=0.05):
    alpha = 1 - np.exp(-s)
    L = np.zeros(len(price))
    L[0] = price[0]
    for t in range(1, len(price)):
        L[t] = alpha * price[t] + (1 - alpha) * L[t - 1]
    return L

def gaussian_filter_last_two(price, sigma=1.2):
    n = len(price)
    if n < 2:
        return 0, 0
    idx_1 = np.arange(n)
    weights_1 = np.exp(-((idx_1+1)**2) / (2 * sigma**2))[::-1]
    weights_1 /= np.sum(weights_1)
    g1 = np.sum(price * weights_1)
    price_2 = price[:-1]
    idx_2 = np.arange(n-1)
    weights_2 = np.exp(-((idx_2+1)**2) / (2 * sigma**2))[::-1]
    weights_2 /= np.sum(weights_2)
    g2 = np.sum(price_2 * weights_2)
    return g1, g2

def get_risk_benchmark_state(context):
    required_days = max(g.ma_period, g.lookback_high_low_days)
    lookback = required_days + 30
    end_date = getattr(context, 'previous_date', None)
    if end_date is None:
        return None
    df = jq_get_price(g.risk_benchmark, end_date=end_date, count=lookback,
                   frequency='1d', fields=['close', 'high', 'low'], panel=False)
    if df is None or len(df) < required_days:
        return None
    daily_close = df['close'].values.astype(float)
    daily_high = df['high'].values.astype(float)
    daily_low = df['low'].values.astype(float)
    current_price = float(daily_close[-1])
    intraday_high = current_price
    intraday_low = current_price
    data_source = '昨日日线'
    try:
        today = context.current_dt.date()
        minute_df = jq_get_price(
            g.risk_benchmark, start_date=today, end_date=context.current_dt,
            frequency='1m', fields=['close', 'high', 'low'],
            panel=False, fill_paused=False
        )
        if minute_df is not None and not minute_df.empty:
            minute_close = minute_df['close'].dropna()
            minute_high = minute_df['high'].dropna()
            minute_low = minute_df['low'].dropna()
            if not minute_close.empty:
                current_price = float(minute_close.iloc[-1])
                intraday_high = float(minute_high.max()) if not minute_high.empty else current_price
                intraday_low = float(minute_low.min()) if not minute_low.empty else current_price
                data_source = '当日盘中'
    except Exception:
        pass
    if current_price <= 0:
        try:
            current_data = get_current_data()
            live_price = current_data[g.risk_benchmark].last_price
            if live_price is not None and live_price > 0:
                current_price = float(live_price)
                intraday_high = max(intraday_high, current_price)
                intraday_low = min(intraday_low, current_price)
                data_source = '实时快照'
        except Exception:
            current_price = float(daily_close[-1])
    close_series = np.append(daily_close, current_price)
    high_series = np.append(daily_high, max(intraday_high, current_price))
    low_series = np.append(daily_low, min(intraday_low, current_price))
    recent_high = np.max(high_series[-g.lookback_high_low_days:])
    recent_low = np.min(low_series[-g.lookback_high_low_days:])
    ma = np.mean(close_series[-g.ma_period:])
    current_rsi = calculate_rsi(close_series, period=14)
    previous_rsi = calculate_rsi(daily_close, period=14)
    return {
        'close_series': close_series,
        'high_series': high_series,
        'low_series': low_series,
        'current_price': current_price,
        'recent_high': recent_high,
        'recent_low': recent_low,
        'ma': ma,
        'current_rsi': current_rsi,
        'previous_rsi': previous_rsi,
        'data_source': data_source,
    }

def is_fresh_stop_loss_signal(context):
    signal_date = getattr(g, 'stop_loss_triggered_date', None)
    if signal_date is None:
        return False
    today = context.current_dt.date()
    previous_date = getattr(context, 'previous_date', None)
    if signal_date == today:
        return True
    if previous_date is not None and signal_date == previous_date:
        return True
    g.stop_loss_triggered_today = False
    g.stop_loss_triggered_date = None
    return False

def init_range_bound_status(context):
    if not g.enable_range_bound_mode:
        return
    log.info("【首次运行】初始化震荡期状态...")
    try:
        if context.previous_date is None:
            log.warning("【首次运行】无法获取前一个交易日，保持正常期")
            return
        end_date = context.previous_date
        lookback = max(g.ma_period, g.lookback_high_low_days) + 30
        df = jq_get_price(g.risk_benchmark, end_date=end_date, count=lookback,
                       frequency='1d', fields=['close', 'high', 'low'], panel=False)
        if df is None or len(df) < max(g.ma_period, g.lookback_high_low_days):
            log.warning("【首次运行】数据不足，保持正常期")
            return
        close = df['close'].values
        high = df['high'].values
        low = df['low'].values
        current_price = close[-1]
        if len(close) >= g.lookback_high_low_days:
            recent_high = np.max(high[-g.lookback_high_low_days:])
            recent_low = np.min(low[-g.lookback_high_low_days:])
        else:
            recent_high = np.max(high)
            recent_low = np.min(low)
        ma = np.mean(close[-g.ma_period:])
        bias = (current_price - ma) / ma if ma > 0 else 0
        rise_from_low = (current_price - recent_low) / recent_low if recent_low > 0 else 0
        current_rsi = calculate_rsi(close, period=14)
        should_enter = False
        signals = []
        if g.enable_bias_trigger and bias > g.bias_threshold:
            should_enter = True
            signals.append('乖离率{:.2%}>{:.0%}'.format(bias, g.bias_threshold))
        if g.enable_rsi_trigger and current_rsi is not None and len(close) >= 15:
            prev_rsi = calculate_rsi(close[:-1], period=14)
            if prev_rsi is not None and prev_rsi > g.rsi_overbought and current_rsi < g.rsi_pullback:
                should_enter = True
                signals.append('RSI超买回落{:.1f}->{:.1f}'.format(prev_rsi, current_rsi))
        if should_enter:
            g.current_filter = '震荡期'
            g.risk_state = '震荡期'
            g.range_bound_start_date = end_date
            g.range_bound_days_count = 0
            log.info('【首次运行】初始化进入震荡期: {}'.format('; '.join(signals)))
        else:
            g.current_filter = '正常期'
            g.risk_state = '正常期'
            if len(close) >= g.lookback_high_low_days:
                g.previous_drawdown = (recent_high - current_price) / recent_high if recent_high > 0 else 0
            else:
                g.previous_drawdown = 0
            g.previous_rsi = current_rsi
            rsi_str = '{:.1f}'.format(current_rsi) if current_rsi is not None else "N/A"
            log.info('【首次运行】初始状态: 正常期, 乖离率: {:.2%}, RSI: {}, 从低点涨幅: {:.2%}'.format(bias, rsi_str, rise_from_low))
    except Exception as e:
        log.warning('【首次运行】初始化震荡期状态异常: {}，保持正常期'.format(e))

def check_and_exit_range_bound_mode(context):
    if not g.enable_range_bound_mode:
        return
    if g.current_filter != '震荡期':
        return
    log.info("【震荡期退出检查】开始检测退出条件...")
    try:
        benchmark_state = get_risk_benchmark_state(context)
        if benchmark_state is None:
            log.warning("【震荡期退出检查】数据不足，跳过")
            return
        close = benchmark_state['close_series']
        current_price = benchmark_state['current_price']
        recent_high = benchmark_state['recent_high']
        recent_low = benchmark_state['recent_low']
        current_drawdown = (recent_high - current_price) / recent_high if recent_high > 0 else 0
        rise_from_low = (current_price - recent_low) / recent_low if recent_low > 0 else 0
        recovery_signals = []
        ma = benchmark_state['ma']
        current_rsi = benchmark_state['current_rsi']
        log.info('【震荡期数据】当前价: {:.3f}, 近{}日高点: {:.3f}, 低点: {:.3f}'.format(current_price, g.lookback_high_low_days, recent_high, recent_low))
        log.info('【震荡期数据】回撤: {:.2%}, 从低点涨幅: {:.2%}'.format(current_drawdown, rise_from_low))
        if g.enable_low_point_rise_trigger:
            if rise_from_low >= g.low_point_rise_threshold:
                recovery_signals.append('从低点上涨{:.2%}>={:.0%}'.format(rise_from_low, g.low_point_rise_threshold))
                log.info('【退出条件触发】从低点上涨: {:.2%}'.format(rise_from_low))
        if g.enable_stable_signal_trigger:
            if current_price > ma:
                recovery_signals.append("价格站上均线")
            if len(close) >= 2 and close[-1] > close[-2]:
                recovery_signals.append("价格回升")
            if g.previous_drawdown is not None and current_drawdown < g.previous_drawdown:
                recovery_signals.append('回撤收窄({:.2%}<{:.2%})'.format(current_drawdown, g.previous_drawdown))
            if current_rsi is not None and g.previous_rsi is not None and current_rsi > g.previous_rsi:
                recovery_signals.append('RSI回升({:.1f})'.format(current_rsi))
            drawdown_safe = current_drawdown < g.drawdown_recovery
            if drawdown_safe:
                g.stable_days += 1
                log.info('【企稳计数】连续企稳天数: {}'.format(g.stable_days))
            else:
                g.stable_days = 0
        g.previous_drawdown = current_drawdown
        g.previous_rsi = current_rsi
        range_bound_days = 0
        if g.range_bound_start_date is not None:
            trade_days = jq_get_trade_days(start_date=g.range_bound_start_date, end_date=context.current_dt.date())
            range_bound_days = len(trade_days) - 1
            if range_bound_days >= g.max_range_bound_days:
                recovery_signals.append('震荡期满({}天)'.format(range_bound_days))
                log.info('【退出条件触发】震荡期已满{}天'.format(range_bound_days))
        low_point_condition = g.enable_low_point_rise_trigger and rise_from_low >= g.low_point_rise_threshold
        stable_condition = False
        if g.enable_stable_signal_trigger:
            drawdown_safe = current_drawdown < g.drawdown_recovery
            stable_condition = drawdown_safe and len(recovery_signals) >= 2 and g.stable_days >= 2
        force_condition = range_bound_days >= g.max_range_bound_days
        should_recover = low_point_condition or stable_condition or force_condition
        if should_recover:
            can_switch = True
            if g.last_switch_date is not None:
                trade_days = jq_get_trade_days(start_date=g.last_switch_date, end_date=context.current_dt.date())
                days_since = len(trade_days) - 1
                if days_since < g.filter_switch_cooldown:
                    can_switch = False
                    log.info('【震荡期退出】冷却期中，距上次切换{}天'.format(days_since))
            if can_switch:
                g.current_filter = '正常期'
                g.risk_state = '正常期'
                g.last_switch_date = context.current_dt.date()
                g.range_bound_start_date = None
                g.range_bound_days_count = 0
                g.stable_days = 0
                log.info('【退出震荡期】切换回拉普拉斯滤波器: {}'.format('; '.join(recovery_signals)))
        else:
            log.info("【震荡期退出检查】未满足退出条件，保持震荡期(高斯滤波器)")
    except Exception as e:
        log.warning('【震荡期退出检查】判断出错: {}'.format(e))

def check_and_enter_range_bound_mode(context):
    if not g.enable_range_bound_mode:
        return
    log.info("【震荡期进入检查】开始检测...")
    stop_loss_signal_active = is_fresh_stop_loss_signal(context)
    can_switch = True
    if g.last_switch_date is not None:
        trade_days = jq_get_trade_days(start_date=g.last_switch_date, end_date=context.current_dt.date())
        days_since = len(trade_days) - 1
        if days_since < g.filter_switch_cooldown:
            can_switch = False
            log.info('【震荡期检查】冷却期中，距上次切换{}天'.format(days_since))
    if g.current_filter == '震荡期':
        log.info("【震荡期检查】当前已在震荡期")
        return
    if not can_switch:
        return
    risk_signals = []
    try:
        benchmark_state = get_risk_benchmark_state(context)
        if benchmark_state is not None:
            close = benchmark_state['close_series']
            current_price = benchmark_state['current_price']
            if g.enable_bias_trigger:
                ma = benchmark_state['ma']
                bias = (current_price - ma) / ma if ma > 0 else 0
                if bias > g.bias_threshold:
                    risk_signals.append('乖离率过大({:.2%}>{:.0%})'.format(bias, g.bias_threshold))
                    log.info('【条件触发】乖离率: {:.2%} (数据源:{})'.format(bias, benchmark_state['data_source']))
            if g.enable_rsi_trigger:
                current_rsi = benchmark_state['current_rsi']
                if len(close) >= 15 and current_rsi is not None:
                    prev_rsi = benchmark_state['previous_rsi']
                    if prev_rsi is not None:
                        if prev_rsi > g.rsi_overbought and current_rsi < g.rsi_pullback and current_rsi < prev_rsi:
                            risk_signals.append('RSI超买回落({:.1f}->{:.1f})'.format(prev_rsi, current_rsi))
                            log.info('【条件触发】RSI超买回落: {:.1f}->{:.1f}'.format(prev_rsi, current_rsi))
    except Exception as e:
        log.warning('【震荡期检查】获取基准数据异常: {}'.format(e))
    if g.enable_stop_loss_trigger and stop_loss_signal_active:
        risk_signals.append("盈利保护触发止损")
        log.info("【条件触发】盈利保护触发止损信号")
    if len(risk_signals) > 0:
        g.current_filter = '震荡期'
        g.risk_state = '震荡期'
        g.last_switch_date = context.current_dt.date()
        g.range_bound_start_date = context.current_dt.date()
        g.range_bound_days_count = 0
        g.stable_days = 0
        g.stop_loss_triggered_today = False
        g.stop_loss_triggered_date = None
        log.info('【进入震荡期】切换到高斯滤波器: {}'.format('; '.join(risk_signals)))
    else:
        log.info("【震荡期检查】未满足进入条件，保持正常期(拉普拉斯滤波器)")

def check_range_bound(context):
    if not g.enable_range_bound_mode:
        return
    log.info("========== 震荡期检查开始 ==========")
    log.info('当前状态: {}'.format(g.current_filter))
    check_and_exit_range_bound_mode(context)
    check_and_enter_range_bound_mode(context)
    log.info('检查后状态: {}'.format(g.current_filter))
    g.rankings_cache = {'date': None, 'data': None}
    log.info("========== 震荡期检查完成 ==========")

def reset_range_bound_daily(context):
    if g.current_filter == '震荡期' and g.range_bound_start_date is not None:
        trade_days = jq_get_trade_days(start_date=g.range_bound_start_date, end_date=context.current_dt.date())
        g.range_bound_days_count = len(trade_days) - 1
        log.info('震荡期已持续 {} 个交易日'.format(g.range_bound_days_count))

# ==================== 动态池管理函数 ====================
def check_dynamic_pool_entry(context):
    today = context.current_dt.date()
    if not g.broadcast_history:
        return
    
    entry_days = g.dynamic_pool_entry_days
    pool_days = g.dynamic_pool_days
    
    for etf in set(g.broadcast_history[-1]['etfs']):
        if etf in MASTER_ETF_POOL:
            continue
        
        try:
            if etf in g.dynamic_etf_pool:
                g.dynamic_etf_pool[etf] += 1
                log.info('🔄 动态池续期: {} {} 再次进入前5，持续时间+1，剩余{}天'.format(etf, get_name(etf), g.dynamic_etf_pool[etf]))
                update_etf_pool_with_dynamic()
                continue
            
            prices = jq_get_price(etf, count=6, end_date=today, fields='close')
            if len(prices) < 6:
                log.debug('🔄 {} {} 价格数据不足，跳过'.format(etf, get_name(etf)))
                continue
            
            c = prices['close']
            five_day_return = (c.iloc[-1] - c.iloc[-6]) / c.iloc[-6]
            three_day_return = (c.iloc[-1] - c.iloc[-4]) / c.iloc[-4]
            up_days = sum(1 for i in range(1, 6) if c.iloc[i] > c.iloc[i - 1])
            
            consecutive_days = 0
            for record in reversed(g.broadcast_history):
                if etf in record['etfs']:
                    consecutive_days += 1
                else:
                    break
            
            if consecutive_days < entry_days:
                continue
            
            if five_day_return <= 0.05:
                log.debug('🔄 {} {} 5日涨幅{:.2f}%<=5%，不满足准入条件'.format(etf, get_name(etf), five_day_return * 100))
                continue
            
            if up_days < 3:
                log.debug('🔄 {} {} 5日上涨天数{}<3，不满足准入条件'.format(etf, get_name(etf), up_days))
                continue
            
            if three_day_return < five_day_return * 0.6:
                log.debug('🔄 {} {} 3日涨幅{:.2f}% < 5日涨幅{:.2f}%的60%({:.2f}%)，不满足准入条件'.format(etf, get_name(etf), three_day_return * 100, five_day_return * 100, five_day_return * 0.6 * 100))
                continue
            
            g.dynamic_etf_pool[etf] = pool_days
            log.info('🔄 动态池扩容: {} {} 连续{}天进入前5，5日涨幅{:.2f}%，5日上涨天数{}，3日涨幅{:.2f}%，加入动态池，持续{}天'.format(etf, get_name(etf), consecutive_days, five_day_return * 100, up_days, three_day_return * 100, pool_days))
            update_etf_pool_with_dynamic()
        except Exception as e:
            log.warning('🔄 {} {} 涨幅计算失败: {}'.format(etf, get_name(etf), e))

def manage_dynamic_pool_lifecycle(context):
    today = context.current_dt.date()
    expired_etfs = []
    
    for etf in list(g.dynamic_etf_pool.keys()):
        if etf in MASTER_ETF_POOL:
            del g.dynamic_etf_pool[etf]
            continue
        g.dynamic_etf_pool[etf] -= 1
        if g.dynamic_etf_pool[etf] <= 0:
            expired_etfs.append(etf)
            log.info('🔄 动态池到期: {} {} 从动态池移除'.format(etf, get_name(etf)))
    
    for etf in expired_etfs:
        del g.dynamic_etf_pool[etf]
    
    update_etf_pool_with_dynamic()

def update_etf_pool_with_dynamic():
    dynamic_etfs = list(g.dynamic_etf_pool.keys())
    g.etf_pool = list(dict.fromkeys(MASTER_ETF_POOL + dynamic_etfs))
    g.all_etf_pool = list(set(g.etf_pool + g.industry_etf_pool))
    g.rankings_cache = {'date': None, 'data': None}

def record_broadcast_history(context, top_etfs):
    today = context.current_dt.date()
    if g.broadcast_history and g.broadcast_history[-1]['date'] == today:
        g.broadcast_history[-1]['etfs'] = top_etfs
    else:
        g.broadcast_history.append({'date': today, 'etfs': top_etfs})
        if len(g.broadcast_history) > 30:
            g.broadcast_history = g.broadcast_history[-30:]

# ==================== 📊 Alpha_003 因子计算 ====================
def calculate_alpha003(close, low, high):
    """
    Alpha_003 因子计算：SUM((CLOSE=DELAY(CLOSE,1)?0:CLOSE-(CLOSE>DELAY(CLOSE,1)?MIN(LOW,DELAY(CLOSE,1)):MAX(HIGH,DELAY(CLOSE,1)))),6)
    
    输入：
    close: 收盘价数组（包含当日实时价）
    low: 最低价数组
    high: 最高价数组
    
    输出：
    alpha003_norm: 归一化后的Alpha_003值（除以收盘价转为百分比）
    """
    close_arr = np.array(close, dtype=float)
    low_arr = np.array(low, dtype=float)
    high_arr = np.array(high, dtype=float)
    
    prev_close = np.roll(close_arr, 1)
    prev_close[0] = np.nan
    
    same = close_arr == prev_close
    up = close_arr > prev_close
    
    ref_price = np.where(up, np.minimum(low_arr, prev_close), np.maximum(high_arr, prev_close))
    
    daily = np.where(same, 0, close_arr - ref_price)
    alpha003_raw = np.nansum(daily[-6:])
    
    if close_arr[-1] != 0:
        alpha003_norm = alpha003_raw / close_arr[-1]
    else:
        alpha003_norm = 0.0
    
    return alpha003_norm

# ==================== 📊 Alpha_004 因子计算 ====================
def calculate_alpha004(enddate, index='all'):
    """
    Alpha_004 因子计算：最近20天下跌次数统计
    
    输入：
    enddate: 必选参数，计算哪一天的因子
    index: 默认参数，股票指数，默认为所有股票'all'，'seesaw'表示跷跷板ETF池
    
    输出：
    一个 Series：index 为成分股代码，values 为最近20天下跌次数
    """
    if index == 'all':
        etf_list = g.all_etf_pool
    elif index == 'seesaw':
        etf_list = g.seesaw_etf_pool
    else:
        etf_list = index
    
    factor_results = {}
    
    for etf in etf_list:
        try:
            daily_hist = jq_get_price(etf, end_date=enddate, count=21, frequency='1d', fields=['close'], skip_paused=False)
            if len(daily_hist) < 20:
                continue
            
            closes = daily_hist['close'].values[-20:]
            down_count = 0
            for i in range(1, len(closes)):
                if closes[i] < closes[i-1]:
                    down_count += 1
            
            factor_results[etf] = down_count
        except Exception:
            continue
    
    return pd.Series(factor_results)

# ==================== 辅助函数 ====================
def get_name(security):
    try:
        name_map = get_stock_name(security)
        if isinstance(name_map, dict):
            return name_map.get(security, security)
        return name_map if name_map else security
    except Exception:
        return security

def smart_order_target_value(security, target_value, context):
    data = get_current_data()
    name = get_name(security)
    
    if data[security].paused:
        log.info('{} {} 停牌，跳过'.format(security, name))
        return False
    price = data[security].last_price
    if price == 0:
        log.info('{} {} 当前价格0，跳过'.format(security, name))
        return False
    # 留出约1%现金缓冲：PTrade 回测单笔委托有约99%可用资金上限，
    # 若按满仓价计算会触发"资金不足"自动缩减（如58600→58000），
    # 导致 trade_log 记录数量与实际持仓不一致，污染 P&L。预留缓冲后
    # 请求量=引擎接受量，记录即真实成交。
    reserve = target_value * 0.01 if target_value > 0 else 0
    target_amount = int((target_value - reserve) / price)
    target_amount = (target_amount // 100) * 100
    if target_amount <= 0 and target_value > 0:
        target_amount = 100
    cur_pos = context.portfolio.positions.get(security, None)
    cur_amount = cur_pos.amount if cur_pos else 0
    diff = target_amount - cur_amount
    if diff > 0:
        if data[security].last_price >= data[security].high_limit:
            log.info('{} {} 涨停，跳过买入'.format(security, name))
            return False
    elif diff < 0:
        if data[security].last_price <= data[security].low_limit:
            log.info('{} {} 跌停，跳过卖出'.format(security, name))
            return False
        recent_vol = jq_attribute_history(security, 5, '1m', ['volume'], skip_paused=False)
        if len(recent_vol) > 0 and np.nansum(recent_vol['volume'].values) == 0:
            log.info('🚫 {} {} 最近5分钟无成交量，判定临时停牌，跳过卖出'.format(security, name))
            return False
    trade_val = abs(diff) * price
    if 0 < trade_val < g.min_money:
        log.info('{} {} 交易金额{:.2f} < {}，跳过'.format(security, name, trade_val, g.min_money))
        return False
    if diff < 0:
        closeable = cur_pos.enable_amount if cur_pos else 0
        if closeable == 0:
            log.info('{} {} 当天买入不可卖出'.format(security, name))
            return False
        diff = -min(abs(diff), closeable)
    if diff != 0:
        order_result = order(security, diff)
        if order_result:
            log.info('{} {} {} 数量{} 价格{:.3f}'.format('📥 买入' if diff > 0 else '📤 卖出', security, name, abs(diff), price))
            if diff > 0:
                g.trade_log['records'].append({
                    'time': context.current_dt.strftime('%Y-%m-%d %H:%M:%S'),
                    'security': security,
                    'name': name,
                    'direction': '买入',
                    'amount': abs(diff),
                    'price': price,
                    'value': abs(diff) * price
                })
            else:
                g.trade_log['sell_records'].append({
                    'time': context.current_dt.strftime('%Y-%m-%d %H:%M:%S'),
                    'security': security,
                    'name': name,
                    'direction': '卖出',
                    'amount': abs(diff),
                    'price': price,
                    'value': abs(diff) * price
                })
            return True
        else:
            log.warning('下单失败: {} {} 数量{}'.format(security, name, diff))
            return False
    return False

# ==================== 核心计算模块 ====================
def get_cached_rankings(context):
    today = context.current_dt.date()
    if g.rankings_cache['date'] != today:
        log.info("重新计算ETF排名...")
        ranked = get_ranked_etfs(context)
        g.rankings_cache = {'date': today, 'data': ranked}
    else:
        log.debug("使用缓存的ETF排名")
    return g.rankings_cache['data']

def get_ranked_etfs(context):
    etf_metrics = []
    for etf in g.etf_pool:
        # 🎯 停牌检测：排名计算时过滤
        if is_etf_suspended(etf, context):
            continue
        
        if get_current_data()[etf].paused:
            continue
        
        metrics = calculate_momentum_metrics(context, etf)
        if metrics is not None:
            if g.min_score_threshold < metrics['score'] < g.max_score_threshold:
                etf_metrics.append(metrics)
            else:
                log.debug('{} {} 得分{:.2f}超出阈值，过滤'.format(etf, metrics['etf_name'], metrics['score']))
    etf_metrics.sort(key=lambda x: x['score'], reverse=True)
    return etf_metrics

def calculate_momentum_metrics(context, etf):
    try:
        name = get_name(etf)
        lookback = max(g.lookback_days, g.short_lookback_days) + 20
        prices = jq_attribute_history(etf, lookback, '1d', ['close', 'high'])
        if len(prices) < g.lookback_days:
            log.debug('{} {} 历史数据不足{}天，跳过'.format(etf, name, len(prices)))
            return None
        # 当日最新价取自上一分钟已收盘K线（jq_last_bar，fq='pre' 同复权链），与 price_series 同尺度；
        # 不再用未复权 last_price（曾致年化6855%假排名）。取不到上一分钟K线则剔除该ETF。
        live_df = jq_last_bar(etf, '1m', ['close'])
        if live_df is None or live_df.empty:
            log.debug('{} {} 无法获取当日K线，跳过'.format(etf, name))
            return None
        current_price = float(live_df['close'].iloc[-1])
        if current_price is None or current_price <= 0:
            return None
        _last_close = float(prices["close"].values[-1]) if len(prices) else 0
        if _last_close > 0 and not (0.5 < current_price / _last_close < 2.0):
            log.info('🚫 {} {} 当日价{:.3f}与前收{:.3f}比例{:.2f}x异常，疑似数据错位，从排名中排除'.format(
                etf, name, current_price, _last_close, current_price / _last_close))
            return None
        price_series = np.append(prices["close"].values, current_price)
        # 盈利保护
        if check_profit_protection(etf, context):
            log.info('🚫 {} {} 触发盈利保护，从排名中排除'.format(etf, name))
            return None
        # 溢价率
        if g.enable_premium_filter:
            prev_date = jq_get_trade_days(end_date=context.current_dt.date(), count=2)[0]
            premium, _, _ = get_premium_rate(etf, prev_date)
            if premium is not None:
                if premium > g.premium_threshold:
                    log.info('🚫 {} {} 溢价率{:.2f}% > {:.0f}%，从排名中排除'.format(etf, name, premium * 100, g.premium_threshold * 100))
                    return None
            else:
                log.debug('{} {} 无法获取溢价率，跳过溢价率过滤'.format(etf, name))
        # 成交量
        if g.enable_volume_check:
            vol_ratio = get_volume_ratio(context, etf)
            if vol_ratio is not None:
                annualized = get_annualized_returns(price_series, g.lookback_days)
                if annualized > g.volume_return_limit:
                    log.info('📉 {} {} 成交量放量{:.1f}倍，且年化{:.1f}% > 阈值{:.1f}%，过滤'.format(etf, name, vol_ratio, annualized * 100, g.volume_return_limit * 100))
                    return None
        # 短期动量
        if len(price_series) >= g.short_lookback_days + 1:
            short_return = price_series[-1] / price_series[-(g.short_lookback_days + 1)] - 1
            short_annualized = (1 + short_return) ** (250 / g.short_lookback_days) - 1
        else:
            short_annualized = 0
        if g.use_short_momentum_filter and short_annualized < g.short_momentum_threshold:
            return None
        # 长期动量
        recent = price_series[-(g.lookback_days + 1):]
        y = np.log(recent)
        x = np.arange(len(y))
        weights = np.linspace(1, 2, len(y))
        slope, intercept = np.polyfit(x, y, 1, w=np.sqrt(weights))
        annualized_returns = math.exp(slope * 250) - 1
        y_bar_w = np.sum(weights * y) / np.sum(weights)
        ss_res = np.sum(weights * (y - (slope * x + intercept)) ** 2)
        ss_tot = np.sum(weights * (y - y_bar_w) ** 2)
        r_squared = ss_res / ss_tot if ss_tot != 0 else 0
        score = annualized_returns * r_squared
        # 近3日跌幅
        if len(price_series) >= 4:
            day1 = price_series[-1] / price_series[-2]
            day2 = price_series[-2] / price_series[-3]
            day3 = price_series[-3] / price_series[-4]
            if min(day1, day2, day3) < g.loss:
                log.info('⚠️ {} {} 近3日有单日跌幅超{:.1f}%，直接排除'.format(etf, name, (1 - g.loss) * 100))
                return None
        # 动态滤波器
        if g.enable_range_bound_mode and len(price_series) >= 10:
            try:
                laplace_values = laplace_filter(price_series, s=g.laplace_s_param)
                laplace_slope = laplace_values[-1] - laplace_values[-2] if len(laplace_values) >= 2 else 0
                passed_laplace = (current_price > laplace_values[-1] and laplace_slope > g.laplace_min_slope)
                g1_val, g2_val = gaussian_filter_last_two(price_series, sigma=g.gaussian_sigma)
                gaussian_slope = g1_val - g2_val
                passed_gaussian = (current_price > g1_val and gaussian_slope > g.gaussian_min_slope)
                if g.current_filter == '正常期':
                    passed_filter = passed_laplace
                    filter_name = '拉普拉斯'
                else:
                    passed_filter = passed_gaussian
                    filter_name = '高斯'
                if not passed_filter:
                    return None
            except Exception as e:
                log.debug('{} {} 滤波器计算异常: {}'.format(etf, name, e))
        return {
            'etf': etf,
            'etf_name': name,
            'annualized_returns': annualized_returns,
            'r_squared': r_squared,
            'score': score,
            'current_price': current_price,
            'short_annualized': short_annualized,
        }
    except Exception as e:
        log.warning('计算{} {}时出错: {}'.format(etf, get_name(etf), e))
        return None

def get_annualized_returns(price_series, lookback_days):
    recent = price_series[-(lookback_days + 1):]
    y = np.log(recent)
    x = np.arange(len(y))
    weights = np.linspace(1, 2, len(y))
    slope, _ = np.polyfit(x, y, 1, w=np.sqrt(weights))
    return math.exp(slope * 250) - 1

def get_volume_ratio(context, security, lookback=None, threshold=None):
    lookback = lookback or g.volume_lookback
    threshold = threshold or g.volume_threshold
    try:
        name = get_name(security)
        hist = jq_attribute_history(security, lookback, '1d', ['volume'])
        if hist.empty or len(hist) < lookback:
            return None
        avg_vol = hist['volume'].mean()
        today = context.current_dt.date()
        df_vol = jq_get_price(security, start_date=today, end_date=context.current_dt,
                           frequency='1m', fields=['volume'], skip_paused=False, fq='pre')
        if df_vol is None or df_vol.empty:
            return None
        current_vol = df_vol['volume'].sum()
        ratio = current_vol / avg_vol if avg_vol > 0 else 0
        if ratio > threshold:
            log.debug('{} {} 成交量比{:.2f} > {}'.format(security, name, ratio, threshold))
            return ratio
        return None
    except Exception as e:
        log.warning('成交量计算失败 {}: {}'.format(security, e))
        return None

@time_monitor("卖出操作")
def etf_sell_trade(context):
    log.info("========== 卖出操作开始 ==========")
    
    ranked = get_cached_rankings(context)
    target_etfs = []
    for m in ranked[:g.holdings_num]:
        if m['score'] >= g.min_score_threshold:
            target_etfs.append(m['etf'])
    target_set = set(target_etfs)
    if not target_set:
        log.info("💤 无目标ETF，清仓所有持仓")
    for sec in list(context.portfolio.positions.keys()):
        pos = context.portfolio.positions[sec]
        if pos.amount == 0:
            continue
        
        if sec not in g.etf_pool:
            if smart_order_target_value(sec, 0, context):
                log.info('📤 卖出不在交易池的持仓：{} {}'.format(sec, get_name(sec)))
            continue
        
        if sec not in target_set:
            if smart_order_target_value(sec, 0, context):
                log.info('📤 卖出不在目标的持仓：{} {}'.format(sec, get_name(sec)))
    log.info("========== 卖出操作完成 ==========")

@time_monitor("买入操作")
def etf_buy_trade(context):
    log.info("========== 买入操作开始 ==========")
    ranked = get_cached_rankings(context)
    log.info("=== ETF排名前5 ===")
    for i, m in enumerate(ranked[:5]):
        log.info('排名{}: {} {} 得分{:.4f} 年化{:.2f}% R²={:.4f}'.format(i + 1, m['etf'], m['etf_name'], m['score'], m['annualized_returns'] * 100, m['r_squared']))
    
    target_etfs = []
    for m in ranked:
        if len(target_etfs) >= g.holdings_num:
            break
        etf = m['etf']
        target_etfs.append(etf)
        log.info('🎯 目标ETF {}: {} {} 得分{:.4f}'.format(len(target_etfs), etf, m['etf_name'], m['score']))
    if not target_etfs:
        log.info(" 无目标ETF，保持空仓")
        return
    current_etf_pos = [s for s in context.portfolio.positions if context.portfolio.positions[s].amount > 0]
    pool_positions = [s for s in current_etf_pos if s in g.etf_pool]
    to_sell = [s for s in pool_positions if s not in target_etfs]
    if to_sell:
        to_sell_names = [get_name(s) for s in to_sell]
        log.info('尚有持仓需要卖出：{}，等待卖出完成再买入'.format(list(zip(to_sell, to_sell_names))))
        return
    total_val = context.portfolio.portfolio_value
    target_per_etf = total_val / len(target_etfs)
    for etf in target_etfs:
        current_val = 0
        if etf in context.portfolio.positions:
            pos = context.portfolio.positions[etf]
            if pos.amount > 0:
                current_val = pos.amount * pos.last_sale_price
        if abs(current_val - target_per_etf) > target_per_etf * 0.05 or current_val == 0:
            if smart_order_target_value(etf, target_per_etf, context):
                action = "买入" if current_val < target_per_etf else "调仓"
                log.info('📦 {}：{} {} 目标金额{:.2f}'.format(action, etf, get_name(etf), target_per_etf))
    log.info("========== 买入操作完成 ==========")

# ==================== 📊 盘后总结 ====================
def daily_summary(context):
    log.info("=" * 70)
    log.info('📊 盘后总结 - {}'.format(context.current_dt.strftime('%Y-%m-%d')))
    log.info("=" * 70)

    portfolio = context.portfolio
    total_value = portfolio.portfolio_value
    cash = portfolio.cash
    positions_value = total_value - cash
    position_count = len([s for s, p in portfolio.positions.items() if p.amount > 0])

    if hasattr(g, 'initial_cash') and g.initial_cash is not None and g.initial_cash > 0:
        cum_return = (total_value / g.initial_cash - 1) * 100
    else:
        cum_return = 0.0

    if g.prev_total_value is not None and g.prev_total_value > 0:
        daily_return = (total_value / g.prev_total_value - 1) * 100
    else:
        daily_return = 0.0
    g.prev_total_value = total_value

    log.info('💰 总资产: {:,.2f} | 可用: {:,.2f} | 市值: {:,.2f} | 累计收益: {:+.2f}% | 当日收益: {:+.2f}%'.format(total_value, cash, positions_value, cum_return, daily_return))

    if position_count > 0:
        for sec, pos in portfolio.positions.items():
            if pos.amount == 0:
                continue
            cur_price = pos.last_sale_price
            cost = pos.cost_basis
            pnl_pct = (cur_price / cost - 1) * 100 if cost > 0 else 0
            hold_days = 0
            if hasattr(pos, 'init_time') and pos.init_time:
                try:
                    start_date = pos.init_time.date()
                    end_date = context.current_dt.date()
                    trade_days = jq_get_trade_days(start_date=start_date, end_date=end_date)
                    hold_days = len(trade_days) - 1
                    if hold_days < 0:
                        hold_days = 0
                except:
                    hold_days = 0
            log.info('{} {} | 成本: {:.3f} | 当前: {:.3f} | 收益: {:+.2f}% | 持有: {}天'.format(sec, get_name(sec), cost, cur_price, pnl_pct, hold_days))
    else:
        log.info("📭 当前空仓")

    all_trades = g.trade_log.get('records', []) + g.trade_log.get('sell_records', [])
    if all_trades:
        all_trades.sort(key=lambda x: x['time'])
        log.info("-" * 70)
        log.info("📝 今日交易记录:")
        for trade in all_trades:
            row = ('{} {} | '.format(trade['security'], trade['name']) + '{} | '.format(trade['direction']) + '数量: {} | '.format(trade['amount']) + '价格: {:.3f} | '.format(trade['price']) + '金额: {:.2f}'.format(trade['value']))
            log.info(row)
        g.trade_log['records'] = []
        g.trade_log['sell_records'] = []
    else:
        log.info("📭 今日无交易")
    log.info("-" * 70)
    log.info("⚙️ 策略状态:")
    log.info('  震荡期: {} (滤波器: {})'.format(g.current_filter, '拉普拉斯' if g.current_filter == '正常期' else '高斯'))
    log.info('  动态扩容池: {} 只'.format(len(g.dynamic_etf_pool)))
    for etf, days in g.dynamic_etf_pool.items():
        log.info('    {} {} - 剩余{}天'.format(etf, get_name(etf), days))
    log.info("=" * 70)
    log.info("🧨🐂🚩🐂🚩🐂🚩🚩🚩🚩🚩🚩🚩🚩🚩报告结束 🚩🚩🚩🚩🚩🚩🚩🚩🚩🐂🚩🐂🚩🐂🧨")
    log.info(" ")

# ==================== 📊 开盘前ETF相关性分析 ====================
def get_top20_etfs_by_growth(context, lookback_days=10):
    """获取10日内涨幅最高的top20 ETF（全市场ETF池）"""
    etf_list = []
    all_etfs = g.all_etf_pool
    total_count = len(all_etfs)
    processed_count = 0
    
    for etf in all_etfs:
        try:
            hist = jq_attribute_history(etf, lookback_days + 5, '1d', ['close', 'volume'])
            if len(hist) < lookback_days:
                continue
            closes = hist['close'].values
            volumes = hist['volume'].values
            
            growth = closes[-1] / closes[-lookback_days] - 1
            turnovers = volumes[-lookback_days:] * closes[-lookback_days:]
            avg_turnover = turnovers.mean()
            
            etf_list.append({
                'etf': etf,
                'name': get_name(etf),
                'growth': growth,
                'avg_turnover': avg_turnover,
                'closes': closes[-lookback_days:]
            })
        except Exception:
            continue
        
        processed_count += 1
        if processed_count % 100 == 0:
            log.debug('📊 已处理{}/{}只ETF'.format(processed_count, total_count))
    
    etf_list.sort(key=lambda x: x['growth'], reverse=True)
    log.info('📊 共处理{}只ETF，筛选出{}只有效数据'.format(processed_count, len(etf_list)))
    return etf_list[:20]

def calculate_correlation(etf1_returns, etf2_returns):
    """计算两个ETF之间的涨跌同步程度（相关性检验）"""
    if len(etf1_returns) != len(etf2_returns):
        return {'pearson': 0.0, 'sign_match': 0.0, 'composite': 0.0}
    
    returns1 = np.array(etf1_returns, dtype=float)
    returns2 = np.array(etf2_returns, dtype=float)
    
    mask = ~(np.isnan(returns1) | np.isnan(returns2))
    returns1 = returns1[mask]
    returns2 = returns2[mask]
    
    if len(returns1) < 2:
        return {'pearson': 0.0, 'sign_match': 0.0, 'composite': 0.0}
    
    mean1 = np.mean(returns1)
    mean2 = np.mean(returns2)
    std1 = np.std(returns1)
    std2 = np.std(returns2)
    
    if std1 == 0 or std2 == 0:
        sign_match = np.sum(np.sign(returns1) == np.sign(returns2)) / len(returns1)
        return {'pearson': 0.0, 'sign_match': sign_match, 'composite': sign_match}
    
    covariance = np.mean((returns1 - mean1) * (returns2 - mean2))
    pearson = covariance / (std1 * std2)
    
    sign_match = np.sum(np.sign(returns1) == np.sign(returns2)) / len(returns1)
    
    composite = (pearson + sign_match) / 2
    
    return {'pearson': pearson, 'sign_match': sign_match, 'composite': composite}

@time_monitor("开盘前相关性分析")
def pre_market_correlation_analysis(context):
    """开盘前监测10日内涨幅最高的top20 ETF，进行相关性检验"""
    log.info("=" * 70)
    log.info('📊 开盘前ETF相关性分析 - {}'.format(context.current_dt.strftime('%Y-%m-%d %H:%M')))
    log.info("=" * 70)
    
    g.dragon_head_etf = None
    g.dragon_head_name = None
    g.dragon_family = {}
    
    top20_etfs = get_top20_etfs_by_growth(context, lookback_days=10)
    
    if len(top20_etfs) < 2:
        log.info("📭 ETF数据不足，跳过相关性分析")
        log.info("=" * 70)
        return
    
    reference_etf = top20_etfs[0]
    
    log.info('\n🎯 参考ETF（涨幅Top1）：{} {}'.format(reference_etf['etf'], reference_etf['name']))
    
    ref_closes = reference_etf['closes']
    ref_returns = []
    for i in range(1, len(ref_closes)):
        if ref_closes[i - 1] != 0:
            ref_returns.append(ref_closes[i] / ref_closes[i - 1] - 1)
        else:
            ref_returns.append(0.0)
    
    correlation_results = []
    for item in top20_etfs:
        if item['etf'] == reference_etf['etf']:
            continue
        
        etf_closes = item['closes']
        etf_returns = []
        for i in range(1, len(etf_closes)):
            if etf_closes[i - 1] != 0:
                etf_returns.append(etf_closes[i] / etf_closes[i - 1] - 1)
            else:
                etf_returns.append(0.0)
        
        corr = calculate_correlation(ref_returns, etf_returns)
        correlation_results.append({
            'etf': item['etf'],
            'name': item['name'],
            'growth': item['growth'],
            'pearson': corr['pearson'],
            'sign_match': corr['sign_match'],
            'composite': corr['composite']
        })
    
    correlation_results.sort(key=lambda x: x['composite'], reverse=True)
    
    high_sync_count = sum(1 for item in correlation_results if item['composite'] >= 0.7)
    medium_sync_count = sum(1 for item in correlation_results if 0.4 <= item['composite'] < 0.7)
    
    log.info('\n🔗 与{}涨跌同步程度排名（强同步{}只，中同步{}只）'.format(reference_etf['name'], high_sync_count, medium_sync_count))
    headers = ['排名', 'ETF代码', '名称', '10日涨幅', '皮尔逊相关', '涨跌方向一致', '综合同步']
    widths = [4, 14, 16, 12, 10, 10, 10]
    log.info(format_table_row(headers, widths))
    log.info("-" * 80)
    
    for i, item in enumerate(correlation_results):
        if i >= 10:
            break
        if item['composite'] < 0.4:
            continue
        name_display = item['name'][:12] if len(item['name']) > 12 else item['name']
        sync_level = "强同步" if item['composite'] >= 0.7 else "中同步"
        cols = [
            str(i + 1),
            item['etf'],
            name_display,
            '{:>11.2f}%'.format(item['growth'] * 100),
            '{:>9.1f}%'.format(item['pearson'] * 100),
            '{:>9.1f}%'.format(item['sign_match'] * 100),
            '{:>9.1f}% {}'.format(item['composite'] * 100, sync_level)
        ]
        log.info(format_table_row(cols, widths))
    log.info("-" * 80)
    
    top10_strong = [item for item in correlation_results[:10] if item['composite'] >= 0.7]
    top5_strong = [item for item in correlation_results[:5] if item['composite'] >= 0.7]
    
    try:
        if len(top10_strong) > 5 and len(top5_strong) >= 3:
            g.dragon_head_etf = reference_etf['etf']
            g.dragon_head_name = reference_etf['name']
            dragon_subordinates = [item for item in correlation_results if item['composite'] >= 0.7]
            g.dragon_family = {item['etf']: {'name': item['name'], 'composite': item['composite'], 'growth': item['growth']} for item in dragon_subordinates}
            
            log.info('\n👑 🌟 龙头家族判定结果：{} 所在板块被列为龙头家族！'.format(reference_etf['name']))
            log.info('   判定条件：Top10强同步{}只(>5)，Top5强同步{}只(≥3)'.format(len(top10_strong), len(top5_strong)))
            log.info('   龙头家族成员总数：{}只（含龙头）'.format(len(g.dragon_family) + 1))
            family_list = ['{}({})'.format(g.dragon_head_etf, g.dragon_head_name[:6])]
            for etf in list(g.dragon_family)[:4]:
                name = g.dragon_family[etf]['name']
                family_list.append('{}({})'.format(etf, name[:6]))
            family_str = ', '.join(family_list)
            if len(g.dragon_family) > 4:
                family_str += '...'
            log.info('   龙头家族列表：{}'.format(family_str))
        else:
            log.info('\n👑 龙头家族判定：未达标（Top10强同步{}只，Top5强同步{}只）'.format(len(top10_strong), len(top5_strong)))
    except Exception as e:
        log.error('⚠️ 龙头家族判定过程出现异常：{}'.format(e))
    
    log.info("=" * 70)

# ==================== 🔍 尾盘龙头监测 ====================
def is_stagnant_with_volume(etf, lookback_days=5, context=None):
    """盘后判断单个ETF是否见顶：放量滞涨 或 慢性滞涨见顶"""
    try:
        end_date = context.current_dt.date()
        daily_hist = jq_get_price(etf, end_date=end_date, count=7, frequency='1d', fields=['close', 'volume'], skip_paused=False)
        if len(daily_hist) < 7:
            return False, None, None, None, None, None
        
        today_close = daily_hist['close'].iloc[-1]
        yesterday_close = daily_hist['close'].iloc[-2]
        day_before_yesterday_close = daily_hist['close'].iloc[-3]
        five_days_ago_close = daily_hist['close'].iloc[-6]
        
        today_volume = daily_hist['volume'].iloc[-1]
        yesterday_volume = daily_hist['volume'].iloc[-2]
        day_before_yesterday_volume = daily_hist['volume'].iloc[-3]
        
        today_change = (today_close - yesterday_close) / yesterday_close * 100
        yesterday_change = (yesterday_close - day_before_yesterday_close) / day_before_yesterday_close * 100
        
        today_turnover = today_volume * today_close
        yesterday_turnover = yesterday_volume * yesterday_close
        day_before_yesterday_turnover = day_before_yesterday_volume * day_before_yesterday_close
        
        five_day_growth = (today_close - five_days_ago_close) / five_days_ago_close * 100
        
        turnover_increased = today_turnover > yesterday_turnover if yesterday_turnover > 0 else False
        
        closes = daily_hist['close'].values
        daily_changes = []
        for i in range(1, len(closes)):
            daily_changes.append((closes[i] - closes[i-1]) / closes[i-1] * 100)
        
        recent_changes = daily_changes[-5:]
        down_count = sum(1 for c in recent_changes if c < 0)
        big_down_count = sum(1 for c in recent_changes if c < -3)
        
        condition_vol_stagnant1 = today_change < yesterday_change
        condition_vol_stagnant2 = turnover_increased
        condition_vol_stagnant3 = five_day_growth > 20
        is_vol_stagnant = condition_vol_stagnant1 and condition_vol_stagnant2 and condition_vol_stagnant3
        
        condition_chronic1 = down_count >= 3
        condition_chronic2 = big_down_count >= 2
        condition_chronic3 = today_turnover > yesterday_turnover and yesterday_turnover > day_before_yesterday_turnover
        is_chronic_stagnant = condition_chronic1 and condition_chronic2 and condition_chronic3
        
        is_stagnant = is_vol_stagnant or is_chronic_stagnant
        stagnant_type = "放量滞涨" if is_vol_stagnant else ("慢性滞涨" if is_chronic_stagnant else None)
        
        return is_stagnant, None, today_change, None, turnover_increased, stagnant_type
    except Exception:
        return False, None, None, None, None, None

@time_monitor("盘后龙头监测")
def dragon_stagnant_monitor(context):
    """盘后15:01监测龙头家族是否见顶（放量滞涨或慢性滞涨）"""
    log.info("=" * 70)
    log.info('🔍 盘后龙头监测 - {}'.format(context.current_dt.strftime('%Y-%m-%d %H:%M')))
    log.info("=" * 70)
    
    if g.dragon_head_etf is None or not g.dragon_family:
        log.info("📭 当前无龙头家族，跳过监测")
        log.info("=" * 70)
        return
    
    head_etf = g.dragon_head_etf
    head_name = g.dragon_head_name
    
    all_members = list(g.dragon_family.keys()) + [head_etf]
    stagnant_count = 0
    details = []
    
    for etf in all_members:
        is_stagnant, amplitude, change, gap_down, turnover_increased, stagnant_type = is_stagnant_with_volume(etf, context=context)
        name = get_name(etf)
        
        change_str = '涨幅{:.2f}%'.format(change) if change else "涨幅N/A"
        turnover_str = "成交额放大" if turnover_increased else "成交额未放大"
        
        if is_stagnant:
            stagnant_count += 1
            log.info('   🚨 {} {}: {}, {}, {}'.format(etf, name[:8], stagnant_type, change_str, turnover_str))
            details.append('{} {}: {}, 涨幅{:.2f}%, 成交额放大'.format(etf, name[:8], stagnant_type, change))
        else:
            log.info('   ✅ {} {}: {}, {}'.format(etf, name[:8], change_str, turnover_str))
    
    log.info('👑 龙头家族：{} 板块'.format(head_name))
    log.info('👨\u200d👩\u200d👧\u200d👦 家族成员总数：{}只，见顶信号：{}只'.format(len(all_members), stagnant_count))
    
    if details:
        log.info("📉 见顶信号成员：")
        for detail in details:
            log.info('   {}'.format(detail))
    
    half_count = (len(all_members) + 1) // 2
    if stagnant_count >= half_count:
        log.info('\n🚨 警报！超过一半成员({}/{})出现见顶信号！'.format(stagnant_count, len(all_members)))
        log.info('⚠️ 龙头家族{}放量滞涨提醒（仅播报，不触发制裁）'.format(head_name))
    else:
        log.info('\n✅ 正常状态（需{}只滞涨，当前{}只）'.format(half_count, stagnant_count))
    
    if stagnant_count > 0:
        log.info("\n🔻 资金高切低监测 - 跷跷板ETF池最弱ETF（因子004）：")
        log.info("-" * 70)
        try:
            end_date = context.current_dt.date()
            alpha004_series = calculate_alpha004(end_date, index='seesaw')
            if not alpha004_series.empty:
                weakest_etfs = alpha004_series.sort_values(ascending=False).head(10)
                for i, (etf, factor_value) in enumerate(weakest_etfs.items(), 1):
                    name = get_name(etf)
                    log.info('   {}. {} {}: 20天下跌{}天'.format(i, etf, name[:8], factor_value))
                log.info("⚠️ 龙头见顶时，资金可能从高位流向银行/国企/红利/公用事业/黄金/能源/农业（仅供参考）")
            else:
                log.info("   暂无有效数据")
        except Exception as e:
            log.info('   因子004计算异常：{}'.format(e))
        log.info("-" * 70)
    
    log.info("=" * 70)




# =====================================================================
# Ptrade 生命周期钩子
# =====================================================================
def before_trading_start(context, data):
    """盘前：把 context 挂到原生 g.context，供 get_current_data 适配层取 current_dt。"""
    g.context = context
    # 原 JQ 盘前任务（09:00/09:05/09:10）合并到此单一钩子
    try:
        manage_dynamic_pool_lifecycle(context)
    except Exception as e:
        log.error("盘前 manage_dynamic_pool_lifecycle 异常: %s" % e)
    try:
        check_positions(context)
    except Exception as e:
        log.error("盘前 check_positions 异常: %s" % e)
    try:
        pre_market_correlation_analysis(context)
    except Exception as e:
        log.error("盘前 pre_market_correlation_analysis 异常: %s" % e)


def after_trading_end(context, data):
    """盘后：原 JQ 盘后任务（15:01/15:05/15:10）合并到此钩子。"""
    g.context = context
    try:
        reset_range_bound_daily(context)
    except Exception as e:
        log.error("盘后 reset_range_bound_daily 异常: %s" % e)
    try:
        dragon_stagnant_monitor(context)
    except Exception as e:
        log.error("盘后 dragon_stagnant_monitor 异常: %s" % e)
    try:
        daily_summary(context)
    except Exception as e:
        log.error("盘后 daily_summary 异常: %s" % e)


def handle_data(context, data):
    """每分钟触发：刷新原生 g.context（供 get_current_data 适配层取最新 current_dt）。"""
    g.context = context
